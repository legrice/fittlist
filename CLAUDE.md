...
**Discover is lenses on one directory, not one list.** A People/Studios/Events
segment above a single row of controls: the search box, and the filters across
from it. The box says which lens it's searching. Studios are not followable and
never will be by that control, because you follow a person and a gym is a
...
to group by, so searching the address is how you find a town. They list in name
order, not schedule-first: a directory of places shouldn't rank them by whether
they signed up, and the Schedule tag says which have a week to see. `.disrow`
keeps 96px clear on the name line for the Follow pill, so a studio row has to
take it back or long gym names truncate into empty space.
