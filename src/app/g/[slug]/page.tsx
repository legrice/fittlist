...
export default async function GroupPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams:Promise<{tab?:string;from?:string}> }) {
  ...
  return <div className="pub group-page hasnav" data-mode={await viewerLook()}><div className="profwrap">{viewerId ? <AppChrome userId={viewerId} social /> : <PublicTopBar next={`/g/${slug}`} />}<main className="group-main"><header className="group-hero"><div className="group-seam-top">{group.bannerPhoto && <img className="profile-banner-image" src={group.bannerPhoto} alt=""/>}{manager && <ProfileBannerSetting compact groupId={group.id}/>}<BackLink className="group-header-control group-hero-back" href={backHref} anywhere notUnder={`/g/${slug}`} label="Back"><Icon name="arrow_back" size={23}/></BackLink><h1 className="group-seam-name">{group.name}</h1>{<GroupOverflow id={group.id} slug={slug} name={group.name} canReport={viewerId!==group.ownerUserId}>{manager&&<GroupSettings slug={slug} name={group.name} photo={group.photo} description={group.description??""} visibility={group.visibility as "public"|"unlisted"|"private"} people={invitePeople} pill/>}</GroupOverflow>}</div><ProfilePhotoZoom photo={group.photo} name={group.name} color={avatarColor({id:group.id})} className="group-profile-photo"/><div className="group-hero-copy"><h1 className="group-copy-name">{group.name}</h1><GroupActions slug={slug} name={group.name} initialFavorite={!!favorite} manager={manager} joined={!!membership||group.ownerUserId===viewerId} joinable={group.visibility!=="private"} invitationRole={invitation?.role}><span className="group-about-action"><ProfileInfoAction><p>{group.description||"No description has been added yet."}</p></ProfileInfoAction></span></GroupActions></div></header><GroupHub about={group.description ?? ""} slug={slug} canPost={!!membership||group.ownerUserId===viewerId} viewerId={viewerId} updates={updates} schedule={schedule} members={members} memberPreview={settingsMembers} initialTab={initialTab}/></main></div></div>;
}
