...
export default async function GroupPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams:Promise<{tab?:string;from?:string}> }) {
  ...
  const schedule = <section className="group-section group-schedule-section"><div className="group-section-head"><h2>Schedule</h2>{manager && <GroupAddClass slug={slug} classes={setupClasses} />}</div>{days.length ? <ClassOpener handle=""><CalendarList days={days} /></ClassOpener> : <div className="empty-block group-schedule-empty"><h2>Nothing planned yet</h2><p>{emptyCopy[purpose]}</p></div>}</section>;
  const members = <GroupMembers slug={slug} inviteToken={manager?group.inviteToken:null} members={settingsMembers} people={dashboard?.people ?? []} canManage={manager} viewerId={viewerId} viewerRole={membership?.role ?? (group.ownerUserId===viewerId?"owner":null)}/>;
