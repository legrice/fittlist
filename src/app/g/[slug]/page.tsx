...
export default async function GroupPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams:Promise<{tab?:string;from?:string}> }) {
  ...
  const members = <GroupMembers slug={slug} members={settingsMembers} people={dashboard?.people ?? []} canInvite={manager}/>;
  const initialTab = tab === "updates" ? "updates" : tab === "members" ? "members" : "schedule";
  return <div className="pub group-page hasnav"><div className="profwrap">{viewerId ? <AppChrome userId={viewerId} /> : <PublicTopBar next={`/g/${slug}`} />}<main className="group-main"><header className="group-hero"><div className="group-page-controls"><Link className="group-header-control" href={from==="discover-groups"?"/discover?half=groups":"/saved"} aria-label="Back to groups"><Icon name="arrow_back" size={23} /></Link><div className="group-page-actions"><GroupShareButton slug={slug} name={group.name} />{manager && <GroupSettings slug={slug} name={group.name} photo={group.photo} description={group.description ?? ""} visibility={group.visibility as "public" | "unlisted" | "private"} people={dashboard?.people ?? []} classes={setupClasses} />}</div></div>{group.photo && <img className="group-hero-photo" src={group.photo} alt="" />}<h1>{group.name}</h1>{group.description && <p>{group.description}</p>}<GroupActions slug={slug} name={group.name} initialFavorite={!!favorite} manager={manager} invitationRole={invitation?.role} /></header><GroupHub slug={slug} canPost={!!membership || group.ownerUserId===viewerId} updates={updates} schedule={schedule} members={members} initialTab={initialTab}/></main></div></div>;
}
