...
import { Share2 } from "@/components/PhosphorIcons";
...
export default function DetailHeader({type,name,id,onBack}:{type:"Group"|"Person"|"Studio"|"Class";name:string;id?:string;onBack:()=>void}) {
 ...
 return <><header className={`${styles.detailNav} ${type==="Class"?styles.classDetailNav:""} ${type==="Group"?styles.groupDetailNav:""}`}><BackButton onClick={onBack}/><span aria-hidden="true"/><div><button aria-label={`Share ${type.toLowerCase()}`} onClick={()=>void share()}><Share2 size={21}/></button></div></header>{status&&<p role="status" className={styles.sectionIntro}>{status}</p>}</>;
}
