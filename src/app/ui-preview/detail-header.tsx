...
import { ArrowLeft, Heart, Share2 } from "@/components/PhosphorIcons";
...
export default function DetailHeader({type,name,id,onBack}:{type:"Group"|"Person"|"Studio"|"Class";name:string;id?:string;onBack:()=>void}) {
 ...
 return <><header className={styles.detailNav}><button aria-label="Back" onClick={onBack}><ArrowLeft size={22}/></button><strong>{type}</strong><div><button aria-label={`Share ${type.toLowerCase()}`} onClick={()=>void share()}><Share2 size={21}/></button><button aria-label={`Favorite ${type.toLowerCase()}`} aria-pressed={favorite} onClick={()=>p.setFavorites(v=>favorite?v.filter(k=>k!==key):[...v,key])}><Heart size={21} opacity={favorite?1:0.45}/></button></div></header>{status&&<p role="status" className={styles.sectionIntro}>{status}</p>}</>;
}
