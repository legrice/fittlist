...
export default function Membership({reason}:{reason?:string}) {
 ...
 const base=selected==="studio"?(annual?43200:4999):(annual?6900:799);
 ...
 const confirm=()=>{
  const summary=selected==="studio"?`${studio} · Studio · ${money(total)} simulated ${annual?"annual":"monthly"} payment.`:`Pro · ${money(total)} simulated ${annual?"annual":"monthly"} payment.`;
  ...
  p.setBillingReceipts(v=>[...v,{scope:selected==="studio"?studio:"Pro",amount:money(total),date:new Date().toLocaleDateString()}]);
 ...
 return <div className={styles.membership}>
  ...
  <p>{reason || "Your studio pays for team management. You never need Pro to see or respond to your work schedule."}</p>
  {step==="confirmed"?<><div className={styles.freePlan}><Check/><p role="status">{receipt}</p><p>{selected==="studio"?"Your studio tools are ready. Staff keep free access to their assignments.":"Your promotion tools are unlocked."}</p></div><button className={styles.membershipCta} onClick={()=>setStep("plans")}>Manage memberships</button></>:<>
  {step==="plans"?<>
