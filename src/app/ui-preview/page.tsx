...
function CalendarScreen({ onShare, firstTime, onNavigate, onOpenGroup, appearance, systemDark, onAppearanceChange, returnScreen }: { onShare: () => void; firstTime:boolean; onNavigate:(screen:Screen)=>void; onOpenGroup:(id:string)=>void; appearance:AppearanceMode; systemDark:boolean; onAppearanceChange:(value:AppearanceMode)=>void; returnScreen:Screen }) {
  ...
  return <div className={`${styles.swipeCalendar} ${homeDayPart==="midday"?styles.middayHome:styles.eveningHome}`}>
    <div className={styles.calendarHero}>
      ...
      {firstTime?<div className={`${styles.calendarSummary} ${styles.yourCalendarSummary} ${styles.firstTimeSummary}`}><strong>Your week starts here.</strong><p>Add classes you teach or plan to take. Follow people to discover what’s on their calendars.</p><div className={styles.firstTimeActions}><button type="button" onClick={()=>p.open({kind:"add-class"})}>Add a class</button></div></div>:<div className={`${styles.calendarSummary} ${styles.yourCalendarSummary}`}><strong>{calendarActivitySummary({ teaching: p.schedule.filter(c=>c.own ?? (c.coach===p.profile.name || c.coach==="Matt LeGrice")).length, attending: personalClasses.filter(c=>!c.personal && !(c.own ?? (c.coach===p.profile.name || c.coach==="Matt LeGrice"))).length, personal: p.schedule.filter(c=>c.personal).length })}</strong><div className={`${styles.heroActions} ${styles.homeSummaryActions}`}><button type="button" onClick={onShare} className={styles.shareWeekButton}>Share your week</button><button type="button" onClick={openCalendar} className={styles.homeManageButton}>Manage calendar</button></div></div>}
    </div>
