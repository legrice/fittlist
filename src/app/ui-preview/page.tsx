...
function CalendarScreen({ onShare, firstTime, onNavigate, onOpenGroup, appearance, returnScreen }: { onShare: () => void; firstTime:boolean; onNavigate:(screen:Screen)=>void; onOpenGroup:(id:string)=>void; appearance:AppearanceMode; returnScreen:Screen }) {
  const p = usePrototype();
  ...
  useEffect(()=>{const hour=new Date().getHours();setGreeting(hour<12?"Good morning":hour<17?"Good afternoon":"Good evening");setDayPart(hour<12?"morning":hour<17?"midday":"evening");},[]);
  useEffect(()=>{const open=()=>{setCalendarOpen(true);window.dispatchEvent(new Event("preview-subpage"));};window.addEventListener("preview-open-calendar",open);return()=>window.removeEventListener("preview-open-calendar",open);},[]);
  ...
  const openCalendar=()=>{setCalendarOpen(true);window.dispatchEvent(new Event("preview-subpage"));};
  ...
  if(calendarOpen)return <div className={`${styles.subpage} ${styles.calendarWorkspace}`}>
    <div className={styles.calendarWorkspaceNav}><BackButton label={returnScreen==="you"?"Back to Profile":"Back to Home"} onClick={()=>{setCalendarOpen(false);if(returnScreen!=="calendar")onNavigate(returnScreen);}}/><button aria-label="Share my calendar" onClick={onShare}><Share2 size={21}/></button></div>
...
function GroupsScreen({selected,setSelected,firstTime}:{selected:string|null;setSelected:(value:string|null)=>void;firstTime:boolean}) {
  ...
  return <div ref={top}>{leaveGroupId&&<div className={styles.followOverlay} onMouseDown={event=>{if(event.target===event.currentTarget)setLeaveGroupId(null);}}><section className={`${styles.followDialog} ${styles.leaveGroupDialog}`} role="alertdialog" aria-modal="true" aria-labelledby="leave-group-title"><button className={styles.followClose} aria-label="Cancel leaving group" onClick={()=>setLeaveGroupId(null)}><X size={20}/></button><h2 id="leave-group-title">Are you sure you want to leave this group?</h2><p>You’ll no longer see its classes or updates with your groups.</p><button className={styles.leaveGroupConfirm} onClick={confirmLeaveGroup}>Leave group</button><button className={styles.leaveGroupCancel} onClick={()=>setLeaveGroupId(null)}>Stay in group</button></section></div>}
    {!selected ? (discover ? <>
      <div className={styles.discoveryHeader}><BackButton className={styles.backButton} label="Back to Your groups" onClick={()=>setDiscover(false)}/><h1 className={styles.pageTitle}>Discover groups</h1></div>
      ...
      <h1 className={styles.visuallyHidden}>Groups</h1><div className={`${styles.collectionPageHeader} ${styles.directorySearchHeader}`}><label className={styles.search}><Search size={19}/><Input value={groupQuery} onFocus={()=>setDiscover(true)} onChange={event=>{setGroupQuery(event.target.value);setDiscover(true);}} placeholder="Search groups" aria-label="Search groups"/></label><button className={styles.collectionDiscover} aria-label="Filter groups by distance and category" onClick={()=>setDiscover(true)}><SlidersHorizontal size={19}/></button></div>
