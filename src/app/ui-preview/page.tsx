...
function CalendarScreen({ onShare, firstTime, onNavigate, onOpenGroup, appearance, systemDark, onAppearanceChange, returnScreen }: { onShare: () => void; firstTime:boolean; onNavigate:(screen:Screen)=>void; onOpenGroup:(id:string)=>void; appearance:AppearanceMode; systemDark:boolean; onAppearanceChange:(value:AppearanceMode)=>void; returnScreen:Screen }) {
  ...
  const [exploredSetup,setExploredSetup]=useState<string[]>([]);
  ...
  const setupTasks=[
    {title:"Complete your profile",done:!!p.live&&!!(p.profile.photo&&p.profile.bio),icon:<UserRound size={19}/>,action:()=>p.open({kind:"edit-profile"})},
    {title:"Add your first class",done:!!p.live&&p.schedule.some(item=>item.own),icon:<CalendarDays size={19}/>,action:()=>p.open({kind:"add-class"})},
    {title:"Explore people",done:exploredSetup.includes("people"),icon:<Users size={19}/>,action:()=>{setExploredSetup(items=>items.includes("people")?items:[...items,"people"]);onNavigate("people");}},
    {title:"Explore groups",done:exploredSetup.includes("groups"),icon:<MessageCircle size={19}/>,action:()=>{setExploredSetup(items=>items.includes("groups")?items:[...items,"groups"]);onNavigate("groups");}},
  ];
  const setupComplete=setupTasks.filter(task=>task.done).length;
  const showSetup=firstTime&&!setupDismissed;
