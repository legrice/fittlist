...
function CalendarScreen({ onShare, firstTime, onNavigate, onOpenGroup, appearance, returnScreen }: { onShare: () => void; firstTime:boolean; onNavigate:(screen:Screen)=>void; onOpenGroup:(id:string)=>void; appearance:AppearanceMode; returnScreen:Screen }) {
  ...
  return <div className={`${styles.swipeCalendar} ${homeDayPart==="morning"?styles.morningHome:homeDayPart==="midday"?styles.middayHome:styles.eveningHome}`}>
    <div className={styles.calendarHero}>
      <div className={styles.homeTopRow}><div className={styles.homeNavIdentity}><button aria-label="Open profile menu" aria-expanded={homeMenuOpen} onClick={()=>setHomeMenuOpen(value=>!value)}><List size={21}/></button><h1 className={styles.homeTitle}>Home</h1></div><div className={styles.homeUtilities}><button aria-label="Notifications" onClick={()=>{p.setSampleNotificationsSeen(true);p.open({kind:"notifications"});}}><Bell size={21}/>{!p.live&&!p.sampleNotificationsSeen&&<span aria-label="2 unread notifications">2</span>}</button><button aria-label="Messages" onClick={()=>{p.setSampleMessagesSeen(true);p.open({kind:"messages"});}}><MessageCircle size={21}/>{!p.live&&!p.sampleMessagesSeen&&<span aria-label="1 unread message">1</span>}</button></div></div>
      {homeMenuOpen&&<div className={styles.homeAccountOverlay} onMouseDown={event=>{if(event.target===event.currentTarget)setHomeMenuOpen(false);}}><aside className={styles.homeAccountDrawer} aria-label="Profile menu"><div className={styles.homeAccountHeader}><Face initials={p.profile.name.split(/\s+/).filter(Boolean).map(part=>part[0]).slice(0,2).join("")} color="#C8C3DB" photo={p.profile.photo} size={54}/><div><strong>{p.profile.name}</strong><span>@{p.profile.handle}</span></div><button aria-label="Close profile menu" onClick={()=>setHomeMenuOpen(false)}><X size={20}/></button></div><div className={styles.homeAccountLinks}><button onClick={()=>{setHomeMenuOpen(false);onNavigate("you");}}><UserRound size={20}/><span><strong>View profile</strong><small>Edit your profile and sharing</small></span><ChevronRight size={17}/></button><button onClick={()=>{setHomeMenuOpen(false);p.open({kind:"membership"});}}><CreditCard size={20}/><span><strong>Membership</strong><small>{p.membershipPlan === "studio" ? "Studio" : p.pro ? "Pro" : "Free"} plan</small></span><ChevronRight size={17}/></button>{[{icon:CalendarDays,title:"Calendar & sync"},{icon:GlobeLock,title:"Privacy & communication"},{icon:Bell,title:"Notification settings"},{icon:LockKeyhole,title:"Account & preferences"}].map(({icon:Icon,title})=><button key={title} onClick={()=>{setHomeMenuOpen(false);p.open({kind:"settings",name:title});}}><Icon size={20}/><span><strong>{title}</strong></span><ChevronRight size={17}/></button>)}<button onClick={()=>{setHomeMenuOpen(false);onNavigate("you");}}><Moon size={20}/><span><strong>Appearance</strong><small>{appearance[0].toUpperCase()+appearance.slice(1)}</small></span><ChevronRight size={17}/></button></div><form action={logout} onSubmit={clearClientMemory}><button type="submit"><LogOut size={20}/>Log out</button></form></aside></div>}
...
function GroupsScreen({selected,setSelected,firstTime}:{selected:string|null;setSelected:(value:string|null)=>void;firstTime:boolean}) {
  ...
  return <div ref={top}>{leaveGroupId&&<div className={styles.followOverlay} onMouseDown={event=>{if(event.target===event.currentTarget)setLeaveGroupId(null);}}><section className={`${styles.followDialog} ${styles.leaveGroupDialog}`} role="alertdialog" aria-modal="true" aria-labelledby="leave-group-title"><button className={styles.followClose} aria-label="Cancel leaving group" onClick={()=>setLeaveGroupId(null)}><X size={20}/></button><h2 id="leave-group-title">Are you sure you want to leave this group?</h2><p>You’ll no longer see its classes or updates with your groups.</p><button className={styles.leaveGroupConfirm} onClick={confirmLeaveGroup}>Leave group</button><button className={styles.leaveGroupCancel} onClick={()=>setLeaveGroupId(null)}>Stay in group</button></section></div>}
    {!selected ? (discover ? <>
      ...
      <div className={styles.collectionPageHeader}><h1 className={styles.pageTitle}>Groups</h1><div className={styles.collectionHeaderActions}><button className={styles.collectionDiscover} aria-label="Search groups" onClick={()=>setDiscover(true)}><Search size={19}/></button><button className={styles.collectionDiscover} aria-label="Filter groups by distance and category" onClick={()=>setDiscover(true)}><SlidersHorizontal size={19}/></button><button className={styles.collectionDiscover} aria-label="Create a group" onClick={()=>setSelected("create")}><Plus size={20}/></button></div></div>
