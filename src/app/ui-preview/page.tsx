...
function CalendarScreen({ onShare, firstTime, onNavigate, onOpenGroup, appearance, returnScreen }: { onShare: () => void; firstTime:boolean; onNavigate:(screen:Screen)=>void; onOpenGroup:(id:string)=>void; appearance:AppearanceMode; returnScreen:Screen }) {
  ...
  return <div className={`${styles.swipeCalendar} ${homeDayPart==="morning"?styles.morningHome:homeDayPart==="midday"?styles.middayHome:styles.eveningHome}`}>
    <div className={styles.calendarHero}>
      <div className={styles.homeTopRow}><div className={styles.homeNavIdentity}><button aria-label="Open navigation menu" aria-expanded={homeMenuOpen} onClick={()=>setHomeMenuOpen(value=>!value)}><List size={21}/></button><h1 className={styles.homeTitle}>Home</h1></div><div className={styles.homeUtilities}><button aria-label="Notifications" onClick={()=>{p.setSampleNotificationsSeen(true);p.open({kind:"notifications"});}}><Bell size={21}/>{!p.live&&!p.sampleNotificationsSeen&&<span aria-label="2 unread notifications">2</span>}</button><button aria-label="Messages" onClick={()=>{p.setSampleMessagesSeen(true);p.open({kind:"messages"});}}><MessageCircle size={21}/>{!p.live&&!p.sampleMessagesSeen&&<span aria-label="1 unread message">1</span>}</button></div></div>
      {homeMenuOpen&&<div className={styles.homeNavMenu} role="menu" aria-label="Navigate FittList"><button role="menuitem" onClick={()=>setHomeMenuOpen(false)}><House size={18}/>Home</button><button role="menuitem" onClick={()=>{setHomeMenuOpen(false);onNavigate("people");}}><UserRound size={18}/>People</button><button role="menuitem" onClick={()=>{setHomeMenuOpen(false);onNavigate("groups");}}><Users size={18}/>Groups</button><button role="menuitem" onClick={()=>{setHomeMenuOpen(false);onNavigate("you");}}><UserRound size={18}/>Profile</button></div>}
