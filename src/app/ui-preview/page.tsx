...
function CalendarScreen({ onShare, firstTime, onNavigate, onOpenGroup, appearance, systemDark, onAppearanceChange, returnScreen }: { onShare: () => void; firstTime:boolean; onNavigate:(screen:Screen)=>void; onOpenGroup:(id:string)=>void; appearance:AppearanceMode; systemDark:boolean; onAppearanceChange:(value:AppearanceMode)=>void; returnScreen:Screen }) {
  const p = usePrototype();
  ...
  const managedStudios=p.live?.managed??[{name:"Ironbound Performance Athletics",initials:"IP",detail:"Studio calendar · You’re an admin",photo:null}];
  const studioTasks=p.live?[]:managedStudios.length?[{title:"Transfer request from Erin Clyne",detail:`${managedStudios[0].name} · Mon, Sep 21`,action:"Review request"},{title:"Assign a coach to Asana Lab",detail:`${managedStudios[0].name} · Sat, Sep 26`,action:"Assign coach"}]:[];
  ...
  const homeDayPart=appearance==="dark"||appearance==="automatic"&&systemDark?"evening":"midday";
  const setupTasks=[
    {title:"Complete your profile",detail:"Add a photo, bio, and coaching focus",done:!!p.live&&!!(p.profile.photo&&p.profile.bio),icon:<UserRound size={19}/>,action:()=>p.open({kind:"edit-profile"})},
    {title:"Add your first class",detail:"Start with a class you teach or plan to take",done:!!p.live&&p.schedule.some(item=>item.own),icon:<CalendarDays size={19}/>,action:()=>p.open({kind:"add-class"})},
    {title:"Follow three people",detail:"Build a useful calendar around your community",done:!!p.live&&p.following.length>=3,icon:<Users size={19}/>,action:()=>onNavigate("people")},
    {title:"Join or create a group",detail:"Make plans and keep your people moving",done:!!p.live&&Object.keys(p.joinedGroups).length>0,icon:<MessageCircle size={19}/>,action:()=>onNavigate("groups")},
  ];
  const setupComplete=setupTasks.filter(task=>task.done).length;
  return <div className={`${styles.swipeCalendar} ${homeDayPart==="midday"?styles.middayHome:styles.eveningHome}`}>
    ...
    {!firstTime&&studioTasks.length>0&&<section className={styles.homeStudioTasks}><SectionTitle aside={<button onClick={()=>p.open({kind:"manage",name:managedStudios[0].name})}>Open studio<ChevronRight size={14}/></button>}>Studio tasks</SectionTitle><div>{studioTasks.map(task=><button key={task.title} onClick={()=>p.open({kind:"manage",name:managedStudios[0].name})}><span className={styles.homeStudioTaskIcon}><Storefront size={18}/></span><span><strong>{task.title}</strong><small>{task.detail}</small></span><em>{task.action}</em><ChevronRight size={16}/></button>)}</div></section>}
    {firstTime?<><section className={styles.setupPanel}><header><div><h2>Finish your setup</h2><p>A few quick steps to make FittList useful for your coaching week.</p></div><span>{setupComplete}/4</span></header><div className={styles.setupTasks}>{setupTasks.map(task=><button key={task.title} data-complete={task.done} onClick={task.action}><span>{task.done?<Check size={18} weight="bold"/>:task.icon}</span><div><strong>{task.title}</strong><small>{task.detail}</small></div><ChevronRight size={17}/></button>)}</div></section><section className={styles.homeDiscovery}><h2>Explore your community</h2><button onClick={()=>onNavigate("people")}><span><UserRound size={20}/></span><div><strong>People near you</strong><small>Follow calendars and discover classes</small></div><ChevronRight size={18}/></button><button onClick={()=>onNavigate("studios")}><span><Storefront size={20}/></span><div><strong>Studios near you</strong><small>Find places you’ll want to revisit</small></div><ChevronRight size={18}/></button><button onClick={()=>onNavigate("groups")}><span><Users size={20}/></span><div><strong>Groups to join</strong><small>Meet people who like to move</small></div><ChevronRight size={18}/></button></section></>:<div className={`${styles.calendarYouPanel} ${styles.homeDashboard}`}><section className={styles.homeCalendarPreview}>{homeWeekClasses.length===0?<p className={styles.sectionIntro}>Nothing scheduled in the next seven days.</p>:homeWeekDates.map(date=><div className={styles.homeClassItem} key={date}><h3>{dateLabel(date)}</h3><div className={styles.dateClassStack}>{homeWeekClasses.filter(item=>item.date===date).map(item=><div className={styles.homeStandaloneClass} key={item.id}><ClassCard item={{...item,day:dateLabel(item.date),color:"#C8C3DB"}}/></div>)}</div></div>)}<button className={styles.homeCardFooter} onClick={openCalendar}>View full calendar<ChevronRight size={14}/></button></section><section className={styles.homeActivitySection}><SectionTitle>Activity</SectionTitle><div className={`${styles.homeActivityList} ${styles.communityList}`}>{activityFeed.map(item=><button data-kind={item.id.split("-")[0]} key={item.id} onClick={item.open}><span>{item.icon}</span><div><strong>{item.title}</strong><small>{item.meta}</small></div><ChevronRight size={17}/></button>)}</div><button className={styles.homeCardFooter} onClick={()=>p.open({kind:"notifications"})}>See all activity<ChevronRight size={14}/></button></section><button onClick={() => p.open({kind:"add-class"})} className={styles.addFab} aria-label="Add a class"><Plus size={28}/></button></div>}
  </div>;
}
...
function ExploreScreen({ page, initialQuery="" }: { page: ExplorePage; initialQuery?:string }) {
  ...
  const searchStarted=query.trim().length>0;
  const controls=<>
    ...
    {page!=="people"&&<div className={styles.directoryFilters} aria-label={`${title} filters`}>
      ...
      <label><select aria-label="Category" value={filter} onChange={event=>setFilter(event.target.value)}><option value="">Category</option>{options.map(option=><option key={option}>{option}</option>)}</select><ChevronDown size={14}/></label>
  ...
  return <div className={styles.directoryWorkspace}>
    ...
    {page==="people"&&!searchStarted?<div className={styles.searchRecents}><h2>Recent searches</h2>{peopleSource.slice(0,3).map(person=><button key={person.name} onClick={()=>setQuery(person.name)}><Search size={17}/><span>{person.name}</span><ChevronRight size={16}/></button>)}</div>:count === 0 && !mapView ? <p className={styles.sectionIntro}>No matches. Try another search.</p>:null}
    <div className={page === "studios" && mapView ? styles.mapResults : undefined}><div aria-hidden={page === "studios" && mapView || undefined} inert={page === "studios" && mapView} className={`${styles.directoryList} ${page === "people" ? styles.peopleList : styles.stack}`}>{page === "people" ? (searchStarted?shownPeople:[]).map(person => <ExplorePerson key={person.name} person={person}/>) : shownStudios.map(place => <ExploreStudio key={place.name} place={place}/>)}</div>{page === "studios" && mapView && <StudioMap studios={shownStudios} filters={controls} onClose={() => setMapView(false)}/>}</div>
...
function GroupsScreen({selected,setSelected,firstTime,dark}:{selected:string|null;setSelected:(value:string|null)=>void;firstTime:boolean;dark:boolean}) {
  const p=usePrototype();
  ...
  return <div ref={top}>{leaveGroupId&&<div className={styles.followOverlay} onMouseDown={event=>{if(event.target===event.currentTarget)setLeaveGroupId(null);}}><section className={`${styles.followDialog} ${styles.leaveGroupDialog}`} role="alertdialog" aria-modal="true" aria-labelledby="leave-group-title"><button className={styles.followClose} aria-label="Cancel leaving group" onClick={()=>setLeaveGroupId(null)}><X size={20}/></button><h2 id="leave-group-title">Are you sure you want to leave this group?</h2><p>You’ll no longer see its classes or updates with your groups.</p><button className={styles.leaveGroupConfirm} onClick={confirmLeaveGroup}>Leave group</button><button className={styles.leaveGroupCancel} onClick={()=>setLeaveGroupId(null)}>Stay in group</button></section></div>}
    {!selected ? (discover ? <>
      ...
      {!groupQuery.trim()?<div className={styles.searchRecents}><h2>Recent searches</h2>{groups.slice(0,3).map(item=><button key={item.id} onClick={()=>setGroupQuery(item.name)}><Search size={17}/><span>{item.name}</span><ChevronRight size={16}/></button>)}</div>:results.length===0?<p className={styles.sectionIntro}>No matching groups. Try another search.</p>:<div className={styles.exploreGroupList}>{results.map(item=><article key={item.id} className={styles.exploreGroupCard}><div className={styles.groupListImage}><button className={styles.groupImageOpen} onClick={()=>openGroup(item.id)} aria-label={`View ${item.name}`}>{item.image?<img src={item.image} alt="" loading="lazy"/>:<Users size={40}/>}</button><span className={styles.groupTypePill}>{categoryOf(item.category)}</span></div><div className={styles.exploreGroupCopy}><div className={styles.groupListTitleRow}><button className={styles.groupListName} onClick={()=>openGroup(item.id)}>{item.name}</button><button className={styles.groupListJoin} aria-pressed={joined.includes(item.id)} aria-label={`${joined.includes(item.id)?"Joined":"Join"} ${item.name}`} onClick={()=>toggleMembership(item.id)}>{joined.includes(item.id)?<><Check size={14} aria-hidden="true"/>Joined</>:"Join"}</button></div><button className={styles.groupListCopyButton} onClick={()=>openGroup(item.id)}><span>{item.location || "Location not added"}</span><span>{item.description}</span></button></div></article>)}</div>}
    ...
    : group && groupView==="members" ? <>
      ...
      <div role="tabpanel" className={styles.groupContentPanel}>{groupView==="updates"?<GroupSampleUpdates live={!!p.live} running={categoryOf(group.category)==="Run club"} dark={dark}/>:group.classIndexes.length ? group.classIndexes.map(index => <section className={styles.daySection} key={index}><h3 className={styles.groupDateTitle}>{groupClasses[index].day}</h3><ClassCard item={groupClasses[index]}/></section>) : <p className={styles.sectionIntro}>{p.live ? "No classes have been added to this group yet." : "No classes planned yet. Check back for the next group plan."}</p>}</div>
      </div>
...
function YouScreen({ appearance, onAppearanceChange, onOpenCalendar, onOpenDetail }: { appearance: AppearanceMode; onAppearanceChange: (value: AppearanceMode) => void; onOpenCalendar: () => void; onOpenDetail?:(destination:Destination)=>void }) {
  ...
  return <div className={styles.youPage}>
    ...
    <section className={styles.profileStudios}><SectionTitle aside={<span>{(p.live?.managed??[{name:"Ironbound Performance Athletics"}]).length}</span>}>Your studios</SectionTitle><div className={styles.profileStudioList}>
      {(p.live?.managed??[{name:"Ironbound Performance Athletics",initials:"IP",detail:"You’re an admin",photo:null},{name:"Asana Soul Practice",initials:"AS",detail:"You’re a coach",photo:null}]).map(studio=><button key={studio.name} className={styles.profileStudioCard} onClick={()=>openDetail({kind:"manage",name:studio.name})}><span className={styles.profileStudioAvatar}>{studio.photo?<img src={studio.photo} alt=""/>:<Storefront size={21}/>}</span><span><strong>{studio.name}</strong><small>{studio.detail.replace("Studio calendar · ","")}</small></span><span className={styles.profileStudioOpen}>Open workspace<ChevronRight size={16}/></span></button>)}
    </div></section>
    <section><SectionTitle>Your calendar</SectionTitle><button className={styles.cardLink} onClick={onOpenCalendar}><Card className={styles.simpleCard}><CardContent className={styles.settingsRow}><span className={styles.settingsIcon}><Face initials={p.profile.name.split(" ").map(part=>part[0]).join("").slice(0,2)} photo={p.profile.photo} color="#C8C3DB" size={40}/></span><div><strong>My calendar</strong><span>Classes, shifts, and saved plans</span></div><ChevronRight size={18}/></CardContent></Card></button></section>
    {[
      { title: "Tools", rows: [
      ...
      { title: "Settings", rows: [
        ...
        { icon: Moon, title: "Appearance", detail: appearance==="automatic"?"Matches your device appearance":appearance==="light"?"Always use the light appearance":"Always use the dark appearance" },
      ] },
      { title: "Admin", rows: [
