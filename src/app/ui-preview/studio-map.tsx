...
export default function StudioMap({studios,onClose}:{studios:MapStudio[];onClose:()=>void}){
 ...
 return <div ref={panel} className={`${styles.mapTakeover} ${styles.libreTakeover} ${styles.inlineStudioMap}`} role="region" aria-label="Studios map view"><div className={styles.studioMapWrap}><div ref={container} className={styles.studioMap}/><button className={styles.mapListButton} data-local-back onClick={close}><List size={18} aria-hidden="true"/>Back to list view</button>{(!tilesReady||failed)&&<p className={styles.mapStatus} role="status">{failed?<>Map could not load. <button onClick={()=>setAttempt(value=>value+1)}>Try again</button></>:"Loading map…"}</p>}
