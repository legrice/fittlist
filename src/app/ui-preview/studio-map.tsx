...
export default function StudioMap({studios,onClose,controls}:{studios:MapStudio[];onClose:()=>void;controls?:ReactNode}){
 const p=usePrototype();const dialog=useRef<HTMLDialogElement>(null),container=useRef<HTMLDivElement>(null),map=useRef<LibreMap|null>(null);
 ...
 const fit=()=>{const rows=latest.current.filter(s=>s.coordinates);if(!rows.length)return;const lngs=rows.map(s=>s.coordinates![1]),lats=rows.map(s=>s.coordinates![0]);map.current?.fitBounds([[Math.min(...lngs),Math.min(...lats)],[Math.max(...lngs),Math.max(...lats)]],{padding:{top:160,bottom:260,left:55,right:55},maxZoom:14,duration:0});};
 useEffect(()=>{dialog.current?.showModal();let disposed=false;let cleanup=()=>{};
 ...
 return <dialog ref={dialog} className={`${styles.mapTakeover} ${styles.libreTakeover}`} aria-label="Studios map view" onClose={onClose}><div className={styles.studioMapWrap}><div ref={container} className={styles.studioMap}/><BackButton autoFocus className={styles.mapBackButton} label="Back to list" onClick={()=>dialog.current?.close()}/><div className={styles.mapFilters}>{controls}</div><button className={styles.mapFit} onClick={fit}>Show all {located.length} studios</button>{(!ready||failed||fallbackUsed)&&<p className={styles.mapStatus} role="status">{failed?'Some map content could not load. You can return to the list.':fallbackUsed?'Using the fallback map.':"Loading map…"}</p>}
 <div className={styles.mapStudioCard} onTouchStart={e=>{touch.current=e.touches[0].clientX;}} onTouchEnd={e=>{if(touch.current!==null){const dx=e.changedTouches[0].clientX-touch.current;if(Math.abs(dx)>50)step(dx<0?1:-1);}touch.current=null;}}>{active?<><div className={styles.mapCardHeading}>{active.photo&&<img src={active.photo} alt=""/>}<div><strong>{active.name}</strong><span>{active.type} · {active.location}</span></div></div><div className={styles.mapCardActions}><button aria-label="Previous studio" onClick={()=>step(-1)} disabled={located.length<2}><ChevronLeft size={20}/></button><button className={styles.mapDetailLink} onClick={()=>{dialog.current?.close();p.open({kind:'studio',name:active.name});}}>View studio</button><button aria-label="Next studio" onClick={()=>step(1)} disabled={located.length<2}><ChevronRight size={20}/></button></div></>:<p>{located.length?'No studios in this area. Move the map or show all studios.':'No mapped studios match these filters.'}</p>}</div>
 </div></dialog>;
}
