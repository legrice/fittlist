...
export default function StudioMap({studios,onClose}:{studios:MapStudio[];onClose:()=>void}){
 ...
 return <div ref={panel} className={`${styles.mapTakeover} ${styles.libreTakeover} ${styles.inlineStudioMap}`} role="region" aria-label="Studios map view"><div className={styles.studioMapWrap}><div ref={container} className={styles.studioMap}/><button className={styles.mapListButton} data-local-back onClick={close}><List size={18} aria-hidden="true"/>Back to list view</button>{(!ready||failed||fallbackUsed)&&<p className={styles.mapStatus} role="status">{failed?'Some map content could not load. You can return to the list.':fallbackUsed?'Using the fallback map.':"Loading map…"}</p>}
