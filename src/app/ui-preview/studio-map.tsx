...
export default function StudioMap({studios,onClose}:{studios:MapStudio[];onClose:()=>void}){
 ...
 return <div ref={panel} className={`${styles.mapTakeover} ${styles.libreTakeover} ${styles.inlineStudioMap}`} role="region" aria-label="Studios map view"><div className={styles.studioMapWrap}><div ref={container} className={styles.studioMap}/><button className={styles.mapListButton} onClick={close}>List view</button>{(!ready||failed||fallbackUsed)&&<p className={styles.mapStatus} role="status">{failed?'Some map content could not load. You can return to the list.':fallbackUsed?'Using the fallback map.':"Loading map…"}</p>}
 <div className={styles.mapStudioCard} onTouchStart={e=>{touch.current=e.touches[0].clientX;}} onTouchEnd={e=>{if(touch.current!==null){const dx=e.changedTouches[0].clientX-touch.current;if(Math.abs(dx)>50)step(dx<0?1:-1);}touch.current=null;}}>{active?<><button className={styles.mapCardHeading} aria-label={`Open ${active.name}`} onClick={()=>p.open({kind:"studio",name:active.name})}>{active.photo&&<img src={active.photo} alt=""/>}<div><strong>{active.name}</strong><span>{active.type} · {active.location}</span></div></button><div className={styles.mapCardActions}><button aria-label="Previous studio" onClick={()=>step(-1)} disabled={located.length<2}><ChevronLeft size={20}/></button><button aria-label="Next studio" onClick={()=>step(1)} disabled={located.length<2}><ChevronRight size={20}/></button></div></>:<p>{located.length?'No studios in this area. Move the map to find studios.':'No mapped studios match these filters.'}</p>}</div>
 </div></div>;
}
