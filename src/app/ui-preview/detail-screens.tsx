...
import { ArrowLeft, ChevronRight, Plus, CalendarDays, Users, MapPin, Bell } from "lucide-react";
import { Button } from "@/components/ui/button";
import { usePrototype, dateLabel, timeLabel, type Destination, type PreviewClass } from "./prototype-state";
...
function Schedule({items}:{items:PreviewClass[]}) { const {open}=usePrototype(); return <div className={styles.stack}>{items.length ? items.map(item=><button key={item.id} className={styles.detailClass} onClick={()=>open({kind:"class",name:item.id})}><small>{dateLabel(item.date)}</small><div><span><strong>{timeLabel(item.time)}</strong><small>{item.duration} min</small></span><span><strong>{item.name}</strong><small>{item.place}</small></span></div></button>) : <p className={styles.sectionIntro}>No upcoming classes yet.</p>}</div>; }
...
function ClassEditor({route}:{route:Destination}) {
  ...
  const [draft,setDraft]=useState<PreviewClass>(existing ?? {id:"",name:"",place:route.name || "Personal calendar",coach:p.profile.name,date:"2026-09-22",time:"18:00",duration:"60"});
...
export default function DetailScreens({route}:{route:Destination}) {
  ...
  const matching=p.schedule.filter(c=>route.kind==="person"?c.coach===route.name:route.name==="Personal calendar"?true:route.name==="Gals who like to move"?["sculpt","ironbound"].includes(c.id)||c.place===route.name:c.place===route.name);
  return <>
    ...
    {(route.kind==="manage"||route.kind==="studio"||route.kind==="person") && <>
      <div className={styles.detailIdentity}><div className={styles.detailAvatar}>{route.kind==="person"?<Users size={30}/>:<CalendarDays size={30}/>}</div><span><MapPin size={15}/> Jersey City, NJ</span></div>
    ...
    {route.kind==="class" && item && <><div className={styles.classDetailFacts}><span>{dateLabel(item.date)}</span><strong>{timeLabel(item.time)} · {item.duration} min</strong><Row title={item.place} detail="View studio" onClick={()=>p.open({kind:"studio",name:item.place})}/><Row title={item.coach} detail="View instructor" onClick={()=>p.open({kind:"person",name:item.coach})}/></div><Heading>About this class</Heading><p className={styles.sectionIntro}>Move together in a welcoming, instructor-led session. Bring water and arrive a few minutes early.</p><div className={styles.profileActions}><Button {...secondary} onClick={()=>p.setSaved(v=>v.includes(item.id)?v.filter(id=>id!==item.id):[...v,item.id])}>{p.saved.includes(item.id)?"Saved":"Save class"}</Button>{(item.place==="Ironbound Performance Athletics"||item.coach===p.profile.name||item.place==="Personal calendar") && <Button {...secondary} onClick={()=>p.open({kind:"edit-class",name:item.id})}>Edit class</Button>}</div></>}
    ...
    <p className={styles.prototypeNote}><Bell size={13}/>Sample data. Changes stay in this preview session.</p>
  </>;
}
