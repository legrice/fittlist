...
import { ChevronRight, Plus, CalendarDays, Users, MapPin, Bell, Clock, Check, ArrowUpRight } from "@/components/PhosphorIcons";
...
function Schedule({items}:{items:PreviewClass[]}) { const dates=[...new Set(items.map(item=>item.date))].sort();return <div className={styles.stack}>{items.length ? dates.map(date=><section key={date} className={styles.daySection}><h3>{dateLabel(date)}</h3><div className={styles.dateClassStack}>{items.filter(item=>item.date===date).map(item=><PreviewClassCard key={item.id} item={item}/>)}</div></section>) : <p className={styles.sectionIntro}>No upcoming classes yet.</p>}</div>; }
...
export default function DetailScreens({route}:{route:Destination}) {
  ...
  return <>
    {route.kind==="add-class"?null:route.kind==="person"||route.kind==="studio" ? <ProfileHero type={route.kind==="person"?"Person":"Studio"} name={title} id={route.name} banner={route.kind==="person"?personAbout?.banner:studioAbout?.banner} photo={route.kind==="person"?personAbout?.photo:studioAbout?.photo} onBack={p.back}/> : route.kind==="class" ? <DetailHeader type="Class" name={title} id={route.name} onBack={p.back}/> : <BackButton className={styles.backButton} onClick={p.back}/>}
    {route.kind!=="person"&&route.kind!=="studio"&&route.kind!=="add-class"&&<h1 className={styles.pageTitle}>{title}</h1>}
    {(route.kind==="notifications"||route.kind==="messages")&&<>
      {route.kind==="notifications"&&!p.live&&<div className={styles.stack}><Row title="Transfer request from Erin Clyne" detail="Ironbound Performance Athletics · Mon, Sep 21" onClick={()=>p.open({kind:"manage",name:"Ironbound Performance Athletics"})}/><Row title="Asana Lab needs a coach" detail="Ironbound Performance Athletics · Sat, Sep 26" onClick={()=>p.open({kind:"manage",name:"Ironbound Performance Athletics"})}/></div>}
      {p.live ? <><p className={styles.sectionIntro}>{title} aren’t connected to this preview yet.</p><a className={styles.backButton} href={route.kind==="notifications"?"/notifications":"/inbox"}>Open {title.toLowerCase()} in the app</a></> : route.kind==="notifications" ? <div className={styles.stack}><div className={styles.followNotification}><button onClick={()=>p.open({kind:"person",name:"Erin Clyne"})}><span><strong>Erin Clyne followed you</strong><small>Today</small></span><ChevronRight size={18}/></button><button aria-pressed={p.following.includes("Erin Clyne")} onClick={()=>{if(!p.following.includes("Erin Clyne"))p.follow("Erin Clyne");}}>{p.following.includes("Erin Clyne")?<><Check size={14}/>Following</>:"Follow back"}</button></div><Row title="Freddie added Sculpt & Strength" detail="Gals who like to move · Yesterday" onClick={()=>p.open({kind:"class",name:"sculpt"})}/><Row title="Alex shared a new class plan" detail="Sunday Sweat Crew · 2 days ago" onClick={()=>p.open({kind:"settings",name:"Groups"})}/><Row title="Jordan saved Morning Flow" detail="Class activity · 3 days ago" onClick={()=>{}}/><Row title="Sam joined your group" detail="Gals who like to move · 4 days ago" onClick={()=>p.open({kind:"settings",name:"Groups"})}/><Row title="Asana Lab posted a new class" detail="Studio update · 5 days ago" onClick={()=>p.open({kind:"studio",name:"Asana Soul Practice"})}/><Row title="Erin commented on your plan" detail="Group activity · 6 days ago" onClick={()=>{}}/><Row title="Your Saturday class is coming up" detail="Calendar reminder · 1 week ago" onClick={()=>{}}/></div> : <div className={styles.stack}><Row title="Erin Clyne" detail="See you at Asana Lab!" onClick={()=>p.open({kind:"conversation",name:"Erin Clyne"})}/></div>}
    </>}
    ...
    {route.kind==="class" && item && <>
      ...
      {!!item.links?.length&&<section className={styles.classPageSection}><Heading>Booking</Heading><div className={styles.bookingLinks}>{item.links.map(link=><a key={`${link.label}-${link.url}`} href={link.url} target="_blank" rel="noreferrer"><span><strong>{link.label||"Book this class"}</strong><small>Continue to the booking site</small></span><ArrowUpRight size={18}/></a>)}</div></section>}
      <div className={styles.classPageFooter}>{(item.own ?? (item.coach===p.profile.name||item.coach==="Matt LeGrice")) ? <button onClick={()=>p.open({kind:"edit-class",name:item.id})}>Edit class</button> : <button aria-pressed={p.saved.includes(item.id)} onClick={()=>p.toggleSaved(item)}>{p.saved.includes(item.id)&&<Check size={20}/>} {p.saved.includes(item.id)?"Saved":"Save class"}</button>}</div>
    </>}
