...
export default function ProfileInfo(p:Props){
 ...
 return <section className={styles.profileInfo}>
  ...
  <div className={styles.profilePills}>{p.action}{p.kind!=="group"&&(p.about||p.extra)&&<button onClick={()=>aboutDialog.current?.showModal()}><Info size={16}/>About</button>}{p.website&&web(p.website)&&<a href={web(p.website)} target="_blank" rel="noreferrer"><Globe size={16}/>Website</a>}{p.kind==="studio"&&dest&&<button onClick={directions}><MapPin size={16}/>Directions</button>}{p.phone&&<a href={`tel:${p.phone.replace(/[^+\d]/g,"")}`}><Phone size={16}/>Call</a>}{p.contactEmail&&<a href={`mailto:${p.contactEmail}`}><Envelope size={16}/>Email</a>}{p.instagram&&<a href={`https://www.instagram.com/${p.instagram.replace(/^https?:\/\/(www\.)?instagram.com\//,"").replace(/^@/,"").split(/[/?#]/)[0]}/`} target="_blank" rel="noreferrer">Instagram</a>}{p.kind!=="group"&&<button onClick={()=>void share()}><Share2 size={16}/>Share</button>}</div>
