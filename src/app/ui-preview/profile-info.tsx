...
export default function ProfileInfo(p:Props){
 const [expanded,setExpanded]=useState(false),[overflow,setOverflow]=useState(false),[miles,setMiles]=useState<number|null>(null),[status,setStatus]=useState("");
 const text=useRef<HTMLParagraphElement>(null),dialog=useRef<HTMLDialogElement>(null);
 useEffect(()=>{const el=text.current;if(!el)return;const check=()=>{if(!expanded)setOverflow(el.scrollHeight>el.clientHeight+1);};check();const observer=new ResizeObserver(check);observer.observe(el);return()=>observer.disconnect();},[p.about,expanded]);
 ...
 return <section className={styles.profileInfo}>
  ...
  {p.about&&<><p ref={text} className={expanded?styles.profileDescription:styles.profileDescriptionClamped}>{p.about}</p>{(overflow||expanded)&&<button className={styles.profileReadMore} aria-expanded={expanded} onClick={()=>setExpanded(!expanded)}>{expanded?"Read less":"Read more"}</button>}</>}
  {expanded&&p.extra&&<div className={styles.profileExtra}>{p.extra}</div>}
  <div className={styles.profilePills}>{p.action}{(p.about||p.extra)&&<button onClick={()=>setExpanded(!expanded)} aria-expanded={expanded}><Info size={16}/>About</button>}{p.website&&web(p.website)&&<a href={web(p.website)} target="_blank" rel="noreferrer"><Globe size={16}/>Website</a>}{p.kind==="studio"&&dest&&<button onClick={directions}><MapPin size={16}/>Directions</button>}{p.phone&&<a href={`tel:${p.phone.replace(/[^+\d]/g,"")}`}><Phone size={16}/>Call</a>}{p.contactEmail&&<a href={`mailto:${p.contactEmail}`}><Envelope size={16}/>Email</a>}{p.instagram&&<a href={`https://www.instagram.com/${p.instagram.replace(/^https?:\/\/(www\.)?instagram.com\//,"").replace(/^@/,"").split(/[/?#]/)[0]}/`} target="_blank" rel="noreferrer">Instagram</a>}<button onClick={()=>void share()}><Share2 size={16}/>Share</button></div>
