...
export async function loadLivePreview() {
  ...
  const [personAbout,studioAbout]=await Promise.all([
    people.people.length?db.select({id:schema.users.id,banner:schema.users.bannerPhoto,about:schema.users.about,website:schema.users.website,phone:schema.users.phone,contactEmail:schema.users.contactEmail,instagram:schema.users.instagram,disciplines:schema.users.disciplines,highlights:schema.users.highlights,certifications:schema.users.certifications}).from(schema.users).where(inArray(schema.users.id,people.people.map(p=>p.id))):[],
    studios.length?db.select({id:schema.studios.id,banner:schema.studios.bannerPhoto,about:schema.studios.about,website:schema.studios.website,phone:schema.studios.phone,contactEmail:schema.studios.contactEmail,instagram:schema.studios.instagram}).from(schema.studios).where(inArray(schema.studios.id,studios.map(s=>s.id))):[],
  ]);
  const memberships=await db.select({id:schema.groups.id,name:schema.groups.name,photo:schema.groups.photo,banner:schema.groups.bannerPhoto,description:schema.groups.description,purpose:schema.groups.purpose,location:schema.users.location,lat:schema.users.locationLat,lng:schema.users.locationLng}).from(schema.groupMembers).innerJoin(schema.groups,eq(schema.groups.id,schema.groupMembers.groupId)).innerJoin(schema.users,eq(schema.users.id,schema.groups.ownerUserId)).where(eq(schema.groupMembers.userId,me.id));
  ...
  const [ownAbout]=await db.select({about:schema.users.about,website:schema.users.website,phone:schema.users.phone,contactEmail:schema.users.contactEmail,instagram:schema.users.instagram}).from(schema.users).where(eq(schema.users.id,me.id));
  return {
    profile:{about:ownAbout?.about||"",website:ownAbout?.website||"",phone:ownAbout?.phone||"",contactEmail:ownAbout?.contactEmail||"",instagram:ownAbout?.instagram||"",name:me.name,handle:me.handle || "",bio:me.title || "",location:me.location || "",email:me.email,photo:me.photo},
    ...
    studios:studios.map(s=>({...studioAbout.find(p=>p.id===s.id),banner:studioAbout.find(p=>p.id===s.id)?.banner || null,about:studioAbout.find(p=>p.id===s.id)?.about || "",name:s.name,placeKind:s.placeKind,types:s.types,type:s.types.join(" · ") || "Studio",location:s.address,photo:s.photo,coordinates:s.lat!=null&&s.lng!=null?[s.lat,s.lng] as [number,number]:null})),
    ...
    groups:[...memberships.map((g,index)=>({id:g.id,name:g.name,image:g.photo || "",banner:g.banner || null,description:g.description || "",category:g.purpose,location:g.location || "",coordinates:g.lat!=null&&g.lng!=null?[g.lat,g.lng] as [number,number]:null,members:memberLists[index].map(m=>m.name),classIndexes:[] as number[]})),...groups.filter(g=>!memberships.some(m=>m.id===g.id)).map(g=>({id:g.id,name:g.name,image:g.photo || "",banner:g.banner || null,description:g.description || "",category:g.purpose,location:g.location || "",coordinates:g.lat!=null&&g.lng!=null?[g.lat,g.lng] as [number,number]:null,members:[] as string[],classIndexes:[] as number[]}))],
