...
import { validPushToken } from "@/lib/fcm";
...
export async function GET(request: Request) {
  ...
  if (!["ios", "android"].includes(platform)) return Response.json({error:"Invalid platform."}, {status:400,headers});
  ...
  return Response.json({ configured: nativePushConfigured(platform), admin: me.admin, enabled: !!device, preferences: device ?? { follows: true, messages: true, updates: true, adminActivity: false } }, { headers });
}
export async function POST(request: Request) {
  ...
  if (raw.length > 8192) return Response.json({ error: "Invalid device." }, { status: 400, headers });
  let input;
  ...
  if (!nativePushConfigured(platform)) return Response.json({ error: "Push notifications are not ready yet. Please try again later." }, { status: 503, headers });
  ...
  if (!validPushToken(platform, input.token)
  ...
  const values = { userId: me.id, token: platform === "ios" ? input.token.toLowerCase() : input.token, platform, sessionVersion: me.version, expiresAt: new Date(exp * 1000),
  ...
  await db.transaction(async tx => {
    ...
    if (input.refresh === true) {
      await tx.update(schema.nativePushDevices).set({ token: values.token, platform: values.platform, sessionVersion: values.sessionVersion, expiresAt: values.expiresAt, updatedAt: values.updatedAt }).where(and(eq(schema.nativePushDevices.id, input.id), eq(schema.nativePushDevices.userId, me.id)));
    } else {
