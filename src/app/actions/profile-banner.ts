...
async function ownedProfile(studioId?: string) {
...
export async function loadProfileBanner(studioId?: string) {
  const profile = await ownedProfile(studioId);
  return profile ? { banner: profile.banner } : null;
}

export async function saveProfileBanner(image: string | null, studioId?: string): Promise<{ ok: boolean; error?: string }> {
  const profile = await ownedProfile(studioId);
  ...
  if (image !== null) {
    ...
    try {
      ...
      banner = await storeImage(`data:image/jpeg;base64,${resized.toString("base64")}`, profile.studio ? "studio" : "u");
    } catch {
      return { ok: false, error: "That image couldn’t be read. Try another photo." };
    }
  }
  const db = await getDb();
  if (profile.studio) await db.update(schema.studios).set({ bannerPhoto: banner }).where(eq(schema.studios.id, profile.id));
