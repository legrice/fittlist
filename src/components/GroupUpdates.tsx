...
function UpdateCard({ slug, update }: { slug:string; update:GroupUpdate }) {
  ...
  return <article className="group-update-card" id={`post-${update.id}`}><header><Avatar person={update.author}/><div><strong>{update.author.name}</strong><small>{new Date(update.createdAt).toLocaleDateString("en-US",{month:"short",day:"numeric"})}</small></div></header>{update.kind==="class_added" && <p className="group-activity-label">Added a class to the calendar</p>}{update.body && <p className="group-update-body">{update.body}</p>}{update.cls && <div className="group-update-class"><div><small>{update.cls.detail}</small><strong>{update.cls.name}</strong><span>{update.cls.where}</span></div><button disabled={pending} className={update.cls.saved?"saved":""} onClick={save}><Icon name={update.cls.saved?"bookmark_added":"bookmark"} size={18}/>{update.cls.saved?"Saved":"Save"}</button></div>}<div className="group-reactions">{([["heart","♥"],["strong","💪"],["in","I’m in"]] as const).map(([key,label])=>{const value=update.reactions.find((r)=>r.reaction===key); return <button className={value?.mine?"on":""} onClick={()=>react(key)} key={key}>{label}{value?.count ? ` ${value.count}`:""}</button>})}</div>{update.comments.length > 0 && <div className="group-comments">{update.comments.map((item)=><div key={item.id}><Avatar person={item.author}/><p><strong>{item.author.name}</strong> {item.body}</p></div>)}</div>}<div className="group-comment-form"><input value={comment} maxLength={300} onChange={(e)=>setComment(e.target.value)} placeholder="Write a reply"/><button disabled={pending || !comment.trim()} onClick={reply} aria-label="Post reply"><Icon name="send" size={18}/></button></div><Toast msg={toastMsg} on={toastOn}/></article>;
}
