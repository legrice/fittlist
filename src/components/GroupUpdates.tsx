...
export type GroupUpdate = {
  ...
  author:{ id:string; name:string; photo:string | null; color:string };
  cls:{ id:string; iso:string; name:string; detail:string; where:string; saved:boolean } | null;
  comments:{ id:string; body:string; author:{ id:string; name:string; photo:string | null; color:string } }[];
...
function GroupUpdates({ slug, canPost, viewerId, updates }: { slug:string; canPost:boolean; viewerId:string|null; updates:GroupUpdate[] }) {
  ...
  return <section className="group-updates">{canPost && <div className="group-composer"><textarea aria-label="Group update" value={text} maxLength={500} onChange={(e)=>setText(e.target.value)} placeholder="Share an update with the group" /><button disabled={pending || !text.trim()} onClick={post}><Icon name="send" size={20} />Post</button></div>}{updates.length ? <div className="group-feed">{updates.map((update)=><UpdateCard slug={slug} update={update} canPost={canPost} viewerId={viewerId} key={update.id} />)}</div> : <div className="group-updates-empty"><Icon name="forum" size={30} /><h2>No updates yet</h2><p>Classes added to the group and member posts will appear here.</p></div>}<Toast msg={toastMsg} on={toastOn} /></section>;
}

function UpdateCard({ slug, update, canPost, viewerId }: { slug:string; update:GroupUpdate; canPost:boolean; viewerId:string|null }) {
  ...
  return <article className="group-update-card" id={`post-${update.id}`}><header><Avatar person={update.author}/><div><strong>{update.author.name}</strong><small>{new Date(update.createdAt).toLocaleDateString("en-US",{month:"short",day:"numeric"})}</small></div>{viewerId && viewerId!==update.author.id && <ReportContentButton contentType="group_post" contentId={update.id} canBlock className="content-report-button group-report-button" />}</header>{update.kind==="class_added" && <p className="group-activity-label">Added a class to the calendar</p>}{update.body && <p className="group-update-body">{update.body}</p>}{update.cls && <div className="group-update-class"><div><small>{update.cls.detail}</small><strong>{update.cls.name}</strong><span>{update.cls.where}</span></div><button disabled={pending} className={update.cls.saved?"saved":""} onClick={save} aria-label={update.cls.saved?`Remove ${update.cls.name} from your calendar`:`Save ${update.cls.name} to your calendar`}><Icon name={update.cls.saved?"bookmark_added":"bookmark"} size={18}/></button></div>}<div className="group-reactions">{([["heart","❤️"],["strong","💪"],["in","I’m in"]] as const).map(([key,label])=>{const value=update.reactions.find((r)=>r.reaction===key); return <button className={value?.mine?"on":""} onClick={()=>react(key)} key={key}>{label}{value?.count ? ` ${value.count}`:""}</button>})}</div>{update.comments.length > 0 && <div className="group-comments">{update.comments.map((item)=><div key={item.id}><Avatar person={item.author}/><p><strong>{item.author.name}</strong> {item.body}</p>{viewerId && viewerId!==item.author.id && <ReportContentButton contentType="group_comment" contentId={item.id} canBlock className="content-report-button group-comment-report" />}</div>)}</div>}{canPost && <div className="group-comment-form"><input aria-label={`Reply to ${update.author.name}`} value={comment} maxLength={300} onChange={(e)=>setComment(e.target.value)} placeholder="Write a reply"/><button disabled={pending || !comment.trim()} onClick={reply} aria-label="Post reply"><Icon name="send" size={18}/></button></div>}<Toast msg={toastMsg} on={toastOn}/></article>;
}
