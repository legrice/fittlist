...
export function FollowingScreen({
  ...
  const [followingSheetExpanded, setFollowingSheetExpanded] = useState(false);
    ...
    <>
      ...
      {isHome && (
        <header className={`calendar-tab-header${activity ? " activity-feed-header" : ""}`}>
          {activity ? <div className="activity-today-hero"><div className="activity-week-topbar">{followingSheetDismissed ? <button type="button" className="activity-restore-sheet" aria-label="Show upcoming classes" onClick={() => { setFollowingSheetDismissed(false); setFollowingSheetY(0); }}><Icon name="close" size={21} /></button> : <span />}<nav className="activity-week-tools" aria-label="Your week tools"><Link href="/discover" aria-label="Search"><Icon name="search" size={25} /></Link><Link href="/notifications" aria-label="Notifications"><Icon name="notifications" size={24} />{unread && <i aria-hidden="true" />}</Link><HeaderAccountButton className="activity-today-avatar" unread={false} face={{ photo:meFace.photo, color:meFace.color, initial:(meFace.name.trim().charAt(0) || "?").toUpperCase() }} /></nav></div><div className="activity-today-date"><div><small className="activity-week-label">Your week</small><div className="activity-today-summary">{weekSummary.classes ? <p>You’re teaching <strong>{weekSummary.classes} {weekSummary.classes === 1 ? "class" : "classes"}</strong> at <strong>{weekSummary.studios} {weekSummary.studios === 1 ? "studio" : "studios"}</strong> this week.</p> : <p>You’re not teaching any classes this week.</p>}</div></div></div><div className="activity-today-hub-actions"><button type="button" onClick={(event) => window.dispatchEvent(new CustomEvent("fittlist:open-share", { detail:{ opener:event.currentTarget } }))}><Icon name="reply" className="share-arrow-forward" size={18} /><span>Share your week</span></button><Link href="/calendar"><Icon name="calendar_month" size={18} /><span>Manage calendar</span></Link></div><GlobalAdd classOnly triggerClassName="activity-hub-add" triggerIconSize={26} onCalendarChange={(focus) => { if (focus) setAddedFocus(focus); }} /></div> : <><button type="button" className="calendar-tab-title" aria-label="Choose a calendar" aria-expanded={calendarSwitcherOpen} onClick={() => setCalendarSwitcherOpen(true)}><h1>Calendar</h1><Icon name="expand_more" size={23} /></button><div className="calendar-tab-actions"><GlobalAdd classOnly triggerClassName="calendar-header-add" triggerIconSize={24} onCalendarChange={(focus) => { if (!focus) return; setIncludeYou(true); setSelectedPeople(new Set()); setCalendarFilter("you"); setCalendarView("day"); setVisibleHomeDayCount(Number.MAX_SAFE_INTEGER); setAddedFocus(focus); }} /></div></>}
        </header>
      )}
      ...
      {activity && <section ref={followingSheetRef} className={`activity-following-sheet${followingSheetExpanded ? " is-expanded" : ""}${followingSheetDismissed ? " is-dismissed" : ""}`} style={{ transform:`translateY(${followingSheetY}px)` }} aria-label="Upcoming from people you follow" onScroll={(event) => setFollowingSheetExpanded(event.currentTarget.scrollTop > 4)} onTouchStart={startFollowingSheetPull} onTouchMove={moveFollowingSheetPull} onTouchEnd={endFollowingSheetPull} onTouchCancel={endFollowingSheetPull}>
