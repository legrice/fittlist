...
export function FollowingScreen({
  ...
  return (
    <>
      ...
      {calendarFollowing && classSheetDismissed && <BodyPortal><div className={`calendar-revealed-controls single${returning ? " is-returning" : ""}`} aria-label="Calendar view"><div className="calendar-view-toggle" data-active={calendarView} role="group" aria-label="Calendar view"><button type="button" aria-label="Day view" aria-pressed={calendarView === "day"} onClick={() => setCalendarView("day")}><Icon name="calendar_view_day" size={22} /></button><button type="button" aria-label="Month view" aria-pressed={calendarView === "month"} onClick={() => setCalendarView("month")}><Icon name="calendar_month" size={22} /></button></div></div></BodyPortal>}
