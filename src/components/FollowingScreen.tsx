...
import { addCalendarActivityComment, toggleCalendarActivityLike } from "@/app/actions/calendar-social";
...
export function FollowingScreen({
  ...
  return (
    <>
      ...
      {calendarFollowing && !classSheetDismissed && <section className={`calendar-action-sheet calendar-following-actions calendar-following-discover calendar-pull-sheet calendar-transition-surface${scopeTarget !== "following" ? " calendar-surface-leaving" : ""}${scopeSummaryEntering ? " calendar-surface-entering" : ""}`} style={{ transform:`translateY(${classSheetPullY}px)` }} onTouchStart={startClassSheetPull} onTouchMove={moveClassSheetPull} onTouchEnd={endClassSheetPull} onTouchCancel={endClassSheetPull} aria-label="Discover people, places, and groups"><div className="calendar-following-surface"><DiscoverList people={[]} studios={[]} cities={[]} groups={[]} upcoming={[]} backHref="/calendar/following" hideBack /></div><footer ref={communityFooterRef} className="calendar-community-footer"><Wordmark variant="cloud" /><p>Thanks for being part of the community.</p><nav aria-label="FittList links"><Link href="/support">Support</Link><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link></nav><small>© {new Date().getFullYear()} FittList</small></footer></section>}
                      ...
                      <div>
                        {section.rows.map((item) => {
                          ...
                          if (calendarFollowing) {
                            ...
                            const actorPhoto=actor?.photo ?? displaySourcePhoto;
                            const actorColor=actor?.color ?? displaySourceColor;
                            const activityKind=item.activityKind ?? "coaching";
                            return <article className="activity-card calendar-following-card" key={item.key} data-cid={item.classId} data-d={item.iso}><button type="button" className="activity-card-main" onClick={() => setPeek(peekOf(item,coach ?? null,favoriteIds.has(item.coachId)))}><span className="activity-card-avatar" style={{ background:actorColor }}>{actorPhoto ? <img src={actorPhoto} alt="" /> : <span>{(storyName.trim().charAt(0)||"?").toUpperCase()}</span>}</span><span className="activity-card-body"><span className="activity-card-story"><strong>{storyName}</strong> is {activityKind === "going" ? "going to" : "coaching"} <b>{item.name}</b></span><span className="activity-card-meta"><span>{item.where || "Location to come"}</span><span>{item.hm}{item.ap.toLowerCase()} · {item.durationMin} min</span></span></span></button>{actor && <div className="calendar-social-actions"><button type="button" className={item.likes?.mine ? "liked" : ""} aria-label={item.likes?.mine ? "Unlike this activity" : "Like this activity"} disabled={socialPending} onClick={() => { const wasLiked=!!item.likes?.mine; setItems((current) => current.map((entry) => entry.key===item.key ? {...entry,likes:{mine:!wasLiked,count:Math.max(0,(entry.likes?.count??0)+(wasLiked?-1:1))}} : entry)); startSocialTransition(async() => { const result=await toggleCalendarActivityLike(actor.id,item.classId,item.iso,activityKind); if (!result.ok) { setItems((current) => current.map((entry) => entry.key===item.key ? {...entry,likes:{mine:wasLiked,count:Math.max(0,(entry.likes?.count??0)+(wasLiked?1:-1))}} : entry)); toast("That activity is no longer available."); } router.refresh(); }); }}><Icon name={item.likes?.mine ? "favorite_filled" : "favorite"} size={20}/>{item.likes?.count ? <span>{item.likes.count}</span> : null}</button><button type="button" aria-label={`Comments on ${storyName}'s activity`} onClick={() => setActivityComments(item)}><Icon name="chat_bubble" size={19}/>{item.comments?.length ? <span>{item.comments.length}</span> : null}</button></div>}</article>;
                          }
      ...
      {calendarFollowing && classSheetDismissed && <BodyPortal><div className="calendar-revealed-controls single" aria-label="Calendar view"><div className="calendar-view-toggle" data-active={calendarView} role="group" aria-label="Calendar view"><button type="button" aria-label="Day view" aria-pressed={calendarView === "day"} onClick={() => setCalendarView("day")}><Icon name="calendar_view_day" size={22} /></button><button type="button" aria-label="Month view" aria-pressed={calendarView === "month"} onClick={() => setCalendarView("month")}><Icon name="calendar_month" size={22} /></button></div></div></BodyPortal>}
      ...
      {activityComments?.activityActor && activityComments.activityKind && <BodyPortal><div className="calendar-activity-scrim" onMouseDown={(event) => { if (event.target===event.currentTarget) setActivityComments(null); }}><section className="calendar-activity-sheet" role="dialog" aria-modal="true" aria-labelledby="calendar-activity-title" onMouseDown={(event) => event.stopPropagation()}><header><div className="calendar-activity-faces"><span style={{background:activityComments.activityActor.color}}>{activityComments.activityActor.photo ? <img src={activityComments.activityActor.photo} alt="" /> : activityComments.activityActor.name.charAt(0)}</span></div><button type="button" aria-label="Close comments" onClick={() => setActivityComments(null)}><Icon name="close" size={21}/></button></header><h2 id="calendar-activity-title"><strong>{activityComments.activityActor.name}</strong> is {activityComments.activityKind === "going" ? "going to" : "coaching"} <b>{activityComments.name}</b></h2><p className="calendar-activity-detail">{daySectionLabel(activityComments.iso,todayIso)} · {activityComments.hm}{activityComments.ap.toLowerCase()} · {activityComments.where || "Location to come"}</p><div className="calendar-activity-comments">{activityComments.comments?.length ? activityComments.comments.map((comment) => <div key={comment.id}><span style={{background:comment.author.color}}>{comment.author.photo ? <img src={comment.author.photo} alt=""/> : comment.author.name.charAt(0)}</span><p><strong>{comment.author.name}</strong>{comment.body}</p><time>{new Date(comment.createdAt).toLocaleDateString("en-US",{month:"short",day:"numeric"})}</time></div>) : <p className="calendar-activity-empty">No comments yet.</p>}</div><form onSubmit={(event) => { event.preventDefault(); const value=activityComment.trim(); if (!value) return; const item=activityComments; startSocialTransition(async() => { const result=await addCalendarActivityComment(item.activityActor!.id,item.classId,item.iso,item.activityKind!,value); if (result.ok) { setActivityComment(""); setActivityComments(null); router.refresh(); } else toast(result.error); }); }}><input value={activityComment} maxLength={300} onChange={(event) => setActivityComment(event.target.value)} placeholder="Add a comment" aria-label="Add a comment"/><button type="submit" disabled={socialPending || !activityComment.trim()} aria-label="Post comment"><Icon name="send" size={19}/></button></form></section></div></BodyPortal>}
