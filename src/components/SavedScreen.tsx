...
export function SavedScreen({ people, places, yourGroups, favoriteGroups, invitations }: { people: YouFavoritePerson[]; places: YouFavoritePlace[]; yourGroups: YouFavoriteGroup[]; favoriteGroups: YouFavoriteGroup[]; invitations: YouGroupInvitation[] }) {
  ...
  return <main className="savedpage">
    ...
    {tab === "groups" ? <div className="saved-groups-view">
      ...
      <section className="saved-block"><div className="saved-block-head"><div><h2>Your groups</h2><p>Groups you own, administer, or joined.</p></div><button type="button" onClick={() => setGroupOpen(true)}><Icon name="add" size={20} />Create group</button></div><div className="saved-group-grid">{yourGroups.map((group) => <GroupCard group={group} key={group.id} />)}<button type="button" className="saved-create-group" onClick={() => setGroupOpen(true)}><Icon name="add" size={28} /><span><strong>Create a group</strong><small>Plan classes with your people</small></span></button></div>{yourGroups.length === 0 && <div className="saved-group-empty"><h3>Plan classes together</h3><p>Create a group, add classes from the catalog, and invite your people.</p></div>}</section>
