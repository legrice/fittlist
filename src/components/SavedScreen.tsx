...
function CreateGroupSheet({ people, classes, onClose }: { people: YouFavoritePerson[]; classes: GroupClassChoice[]; onClose: () => void }) {
  ...
  const submit = () => start(async () => {
    setError("");
    const result = await createGroup({ name, description, memberIds: selected, classes: classes.filter((item) => selectedClasses.includes(`${item.classId}|${item.iso}`)).map(({ classId, iso }) => ({ classId, iso })) });
  ...
  return <div className="sheet-scrim" onClick={(event) => { if (event.target === event.currentTarget && !pending) onClose(); }}>
    <div className="sheet create-group-sheet">
      ...
      <div className="create-group-progress" aria-label={`Step ${step + 1} of 4`}><span>Step {step + 1} of 4</span><div>{[0, 1, 2, 3].map((item) => <i className={item <= step ? "on" : ""} key={item} />)}</div></div>
      {step === 0 && <div className="create-group-step"><h2>Name your group</h2><p>Choose something your friends will recognize.</p><label className="create-group-name"><span>Group name</span><input autoFocus maxLength={60} value={name} onChange={(event) => setName(event.target.value)} placeholder="Saturday run crew" /></label></div>}
