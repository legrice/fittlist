...
export function CreateGroupSheet({ onClose }: { onClose: () => void }) {
  ...
  return <div className="sheet-scrim create-group-scrim" onClick={(event) => { if (event.target === event.currentTarget && !pending) onClose(); }}>
    <div ref={sheetRef} className="sheet create-group-sheet" role="dialog" aria-modal="true" aria-labelledby="create-group-title">
      <button type="button" className="iconbtn sheetclose sheet-dismiss" aria-label="Close" onClick={onClose}><Icon name="close" size={20} /></button>
      ...
      <div className="create-group-actions">{error && <p className="formerror" role="alert">{error}</p>}<div>{step > 0 && <button type="button" className="btn ghost" disabled={pending} onClick={() => setStep((current) => current - 1)}><Icon name="arrow_back" size={22}/>Back</button>}<button type="button" className="btn create-group-submit" disabled={pending || step === 0 && (name.trim().length < 2 || slug.length < 3)} onClick={step === 2 ? submit : next}>{pending ? step === 2 ? "Creating group…" : "Checking link…" : step === 2 ? "Create group" : "Continue"}</button></div></div>
    </div>
  </div>;
}
