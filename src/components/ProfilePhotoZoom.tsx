...
export function ProfilePhotoZoom({ photo, name, color, className }: { photo:string|null; name:string; color:string; className:string }) {
  ...
  useEffect(()=>{
    // The browser may have finished a cached failure before hydration.
  ...
  return <>
    <button ref={trigger} type="button" className={`${className} profile-photo-zoom-trigger`} aria-label={`View ${name} photo`} onClick={()=>{setNotice("");setOpen(true);}}>
      {visiblePhoto?<img ref={photoRef} src={visiblePhoto} alt="" onError={()=>setFailedPhoto(visiblePhoto)}/>:<span style={{background:color}}>{initial}</span>}
    </button>
    {open&&<BodyPortal><div className="profile-photo-viewer" role="dialog" aria-modal="true" aria-label={`${name} photo`} onClick={(event)=>{if(event.target===event.currentTarget)setOpen(false);}}>
      ...
      <div className="photo-viewer-content">
        {visiblePhoto?<img className="photo-viewer-image" src={visiblePhoto} alt={name} onError={()=>setFailedPhoto(visiblePhoto)}/>:<span className="photo-viewer-placeholder" style={{background:color}}>{initial}</span>}
        <div className="photo-viewer-actions">
          ...
          {visiblePhoto&&<a href={visiblePhoto} target="_blank" rel="noopener noreferrer"><Icon name="open_in_new" size={24}/>Open photo</a>}
        </div>
