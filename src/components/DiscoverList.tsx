...
export function DiscoverList({ people,studios=[],cities,myLat=null,myLng=null,startHalf,upcoming=[],groups=[],backHref,hideBack=false }: { people:DirPerson[];studios?:DirStudio[];cities:string[];myCity?:string|null;myLat?:number|null;myLng?:number|null;startHalf?:DiscoverHalf;upcoming?:BrowseDay[];groups?:Group[];backHref:string;hideBack?:boolean }) {
  ...
  const [studioType,setStudioType]=useState(""); const [studioDistance,setStudioDistance]=useState("");
  ...
  return <>
    ...
    {tab==="classes"&&<><FilterRow><Filter label="Type" value={classType} onChange={setClassType} all="All types" options={classTypes}/><Filter label="Distance" value={distance} onChange={setDistance} all="Any distance" options={[["1","Within 1 mile"],["5","Within 5 miles"],["10","Within 10 miles"],["25","Within 25 miles"]]} disabled={myLat==null||myLng==null}/></FilterRow>{discoverCalendarDays.length?<CalendarList className="discover-calendar-list" days={discoverCalendarDays}/>:<Empty>There are no classes matching these filters.</Empty>}</>}
    {tab==="people"&&<><FilterRow><Filter label="Specialty" value={discipline} onChange={setDiscipline} all="All specialties" options={disciplines}/><Filter label="Distance" value={peopleDistance} onChange={setPeopleDistance} all="Any distance" options={[["1","Within 1 mile"],["5","Within 5 miles"],["10","Within 10 miles"],["25","Within 25 miles"]]} disabled={myLat==null||myLng==null}/></FilterRow>{directoryPending?<Empty>Loading people…</Empty>:shownPeople.length?<div className="discover-person-grid">{shownPeople.map((person,index)=><DiscoverPerson person={person} index={index} activity={activityByName.get(person.name.trim().toLowerCase())??person.classesThisWeek} key={person.id}/>)}</div>:<Empty>There are no people matching these filters.</Empty>}</>}
    {tab==="places"&&<><FilterRow><Filter label="Type" value={studioType} onChange={setStudioType} all="All types" options={studioTypes}/><Filter label="Distance" value={studioDistance} onChange={setStudioDistance} all="Any distance" options={[["1","Within 1 mile"],["5","Within 5 miles"],["10","Within 10 miles"],["25","Within 25 miles"]]} disabled={myLat==null||myLng==null}/></FilterRow>{directoryPending?<Empty>Loading studios…</Empty>:shownStudios.length?<StudioGrid studios={shownStudios}/>:<Empty>There are no studios matching these filters.</Empty>}</>}
