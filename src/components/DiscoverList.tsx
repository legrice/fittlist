...
const CreateGroupSheet = dynamic(() => import("@/components/SavedScreen").then(module => module.CreateGroupSheet), { loading: () => <LoadingDots label="Opening group creation" /> });
import { discoverGroups, discoverPeople, discoverStudios } from "@/lib/directory-client";
...
export function DiscoverList({ people,studios=[],cities,myLat=null,myLng=null,startHalf,upcoming=[],groups=[],backHref,hideBack=false,groupFrom="discover-groups" }: { people:DirPerson[];studios?:DirStudio[];cities:string[];myCity?:string|null;myLat?:number|null;myLng?:number|null;startHalf?:DiscoverHalf;upcoming?:BrowseDay[];groups?:Group[];backHref:string;hideBack?:boolean;groupFrom?:"discover-groups"|"calendar-following" }) {
  ...
  const retryDirectory=()=>{loadedDirectories.current.delete(tab);setDirectoryStatus(current=>{const next={...current};delete next[activeDirectoryKey];return next;});setRetry(value=>value+1);};
  ...
  },[tab,peopleDistance,studioDistance,groupDistance,geo,retry]);
  ...
  return <>
    ...
    <div className="discover-directory-tabs" role="tablist" aria-label="Directory type" onKeyDown={tabListKeyDown}>{([['people','People'],['places','Studios'],['groups','Groups']] as const).map(([value,label])=><button type="button" role="tab" tabIndex={tab===value||(tab==="classes"&&value==="people")?0:-1} aria-selected={tab===value} className={tab===value?"on":""} onClick={()=>setTab(value)} key={value}>{label}</button>)}</div>
    {tab==="classes"&&<div className="discover-results-workspace"><FilterRow><Filter label="Distance" value={distance} onChange={(value)=>chooseDistance(value,setDistance)} all="Any distance" options={distanceOptions} disabled={locationPending}/><Filter label="Type" value={classType} onChange={setClassType} all="Any type" options={classTypes}/></FilterRow><div className="discover-results-column">{discoverCalendarDays.length?<ClassOpener handle=""><CalendarList className="discover-calendar-list" days={discoverCalendarDays}/></ClassOpener>:<Empty>There are no classes matching these filters.</Empty>}</div></div>}
    {tab==="people"&&<div className="discover-results-workspace"><FilterRow><Filter label="Distance" value={peopleDistance} onChange={(value)=>chooseDistance(value,setPeopleDistance)} all="No distance limit" options={distanceOptions} disabled={locationPending}/><Filter label="Specialty" value={discipline} onChange={setDiscipline} all="Any specialty" options={disciplines}/></FilterRow><div className="discover-results-column">{directoryLoading?<div className="directory-loading"><LoadingDots label="Loading people…"/></div>:directoryFailed?<Empty>Couldn’t load people. <button type="button" className="ghost" onClick={retryDirectory}>Try again</button></Empty>:shownPeople.length?<div className="discover-person-grid">{shownPeople.map((person,index)=><DiscoverPerson person={person} index={index} activity={activityByName.get(person.name.trim().toLowerCase())??person.classesThisWeek} key={person.id}/>)}</div>:<Empty>There are no people matching these filters.</Empty>}</div></div>}
    {tab==="places"&&<div className="discover-results-workspace"><FilterRow><Filter label="Distance" value={studioDistance} onChange={(value)=>chooseDistance(value,setStudioDistance)} all="Any distance" options={distanceOptions} disabled={locationPending}/><Filter label="Type" value={placeKind} onChange={setPlaceKind} all="Any type" options={PLACE_KINDS.map((kind)=>[kind,PLACE_KIND_LABELS[kind]] as const)}/><Filter label="Category" value={studioType} onChange={setStudioType} all="Any category" options={studioTypes}/></FilterRow><div className="discover-results-column">{directoryLoading?<div className="directory-loading"><LoadingDots label="Loading studios…"/></div>:directoryFailed?<Empty>Couldn’t load studios. <button type="button" className="ghost" onClick={retryDirectory}>Try again</button></Empty>:shownStudios.length?<StudioGrid studios={shownStudios}/>:<Empty>There are no studios matching these filters.</Empty>}</div></div>}
    ...
    {!hideBack&&<Link className="logoutbtn" href={backHref}><Icon name="arrow_back" size={22}/>Back to your week</Link>}
    {groupCreateOpen&&(
...
function FilterRow({children}:{children:ReactNode}){return <aside className="discover-filter-sidebar" aria-label="Discover filters"><h2>Filters</h2><div className="discover-class-filters discover-tab-filters">{children}</div></aside>}
function Filter({label,value,onChange,all,options,disabled=false}:{label:string;value:string;onChange:(value:string)=>void;all:string;options:readonly (string|readonly[string,string])[];disabled?:boolean}){return <label><span className="discover-filter-label">{label}</span><select aria-label={label} value={value} onChange={(event)=>onChange(event.target.value)} disabled={disabled}><option value="">{all}</option>{options.map((option)=>{const [value,label]=typeof option==="string"?[option,option]:option;return <option value={value} key={value}>{label}</option>})}</select></label>}
...
function StudioGrid({studios}:{studios:DirStudio[]}){
  return <div className="discover-studio-grid">{studios.map((studio,index)=><div className="discover-studio-row" key={studio.id}>
    <Link prefetch={false} className="discover-studio-main" href={`/s/${studio.slug}?from=discover-studios`}>
      <span className="discover-studio-media">{studio.photo?<img src={studio.photo} alt="" loading={index<12?"eager":"lazy"} decoding="async"/>:<span className="discover-studio-placeholder" style={{background:studio.color}}>{studio.name.trim().charAt(0).toUpperCase()}</span>}</span>
...
function GroupGrid({groups,from="discover-groups"}:{groups:Group[];from?:"discover-groups"|"calendar-following"}){return <div className="discover-group-list">{groups.map((group)=><div className="discover-group-row" key={group.id}><Link prefetch={false} className="discover-group-main" href={`/g/${group.slug}?from=${from}`}><span className="discover-group-face">{group.photo ? <img src={group.photo} alt="" loading="lazy" decoding="async" /> : <Icon name="groups" size={28}/>}</span><span className="discover-group-copy"><strong>{group.name}</strong><small>{group.description||"Open group"}</small></span></Link><FavoriteGroupButton group={group}/></div>)}</div>}
