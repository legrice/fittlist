...
export function DiscoverList({ people,studios=[],cities,myLat=null,myLng=null,startHalf,upcoming=[],groups=[],backHref,hideBack=false }: { people:DirPerson[];studios?:DirStudio[];cities:string[];myCity?:string|null;myLat?:number|null;myLng?:number|null;startHalf?:DiscoverHalf;upcoming?:BrowseDay[];groups?:Group[];backHref:string;hideBack?:boolean }) {
  ...
  const openEvent=(item:UpcomingItem)=>router.push(`/${item.base.replace(/^\//,"")}/${item.classId}?d=${item.iso}&from=discover-classes`);
  ...
  const saveClass=(item:UpcomingItem)=>startSave(async()=>{const key=`${item.classId}|${item.iso}`;if(saved[key]){await showSavedConfirmation(item);return;}const result=await setGoing(item.classId,item.iso,true);if(!result.ok)return toast(result.error??"Couldn't save that class");setSaved((current)=>({...current,[key]:true}));toast(`${item.name} was saved to your calendar`);await showSavedConfirmation(item);router.refresh();});
  ...
  const discoverCalendarDays:WeekDayRows[]=filteredUpcomingDays.map((day)=>({iso:day.iso,label:day.label,rows:day.items.map((item)=>{const isSaved=!!saved[`${item.classId}|${item.iso}`];return {key:`${item.classId}.${item.iso}`,name:item.name,where:item.where||"Location to come",hm:item.hm,ap:item.ap,coach:{id:item.classId,name:item.attributionName,color:item.coachColor,photo:item.coachPhoto},onTap:()=>void openEvent(item),corner:item.own?undefined:<button type="button" aria-label={isSaved?`Manage saved class ${item.name}`:`Save ${item.name} to your calendar`} className={`calendar-overlay-save${isSaved?" saved":""}`} disabled={savePending} onClick={()=>isSaved?openSavedConfirmation(item):saveClass(item)}><Icon name={isSaved?"bookmark_added":"bookmark"} size={17}/>{isSaved?"Saved":"Save"}</button>};})}));
  useEffect(()=>{
  ...
  return <>
    ...
    {tab==="classes"&&<><FilterRow><Filter label="Type" value={classType} onChange={setClassType} all="Type" options={classTypes}/><Filter label="Distance" value={distance} onChange={setDistance} all="Distance" options={[["1","Within 1 mile"],["5","Within 5 miles"],["10","Within 10 miles"],["25","Within 25 miles"]]} disabled={myLat==null||myLng==null}/></FilterRow>{discoverCalendarDays.length?<CalendarList className="discover-calendar-list" days={discoverCalendarDays}/>:<Empty>There are no classes matching these filters.</Empty>}</>}
    {tab==="people"&&<><FilterRow><Filter label="Specialty" value={discipline} onChange={setDiscipline} all="Specialty" options={disciplines}/><Filter label="Distance" value={peopleDistance} onChange={setPeopleDistance} all="Distance" options={[["1","Within 1 mile"],["5","Within 5 miles"],["10","Within 10 miles"],["25","Within 25 miles"]]} disabled={myLat==null||myLng==null}/></FilterRow>{directoryPending?<Empty>Loading people…</Empty>:shownPeople.length?<div className="discover-person-grid">{shownPeople.map((person,index)=><DiscoverPerson person={person} index={index} activity={activityByName.get(person.name.trim().toLowerCase())??person.classesThisWeek} key={person.id}/>)}</div>:<Empty>There are no people matching these filters.</Empty>}</>}
    {tab==="places"&&<><FilterRow><Filter label="Type" value={placeKind} onChange={setPlaceKind} all="Type" options={PLACE_KINDS.map((kind)=>[kind,PLACE_KIND_LABELS[kind]] as const)}/><Filter label="Category" value={studioType} onChange={setStudioType} all="Category" options={studioTypes}/><Filter label="Distance" value={studioDistance} onChange={setStudioDistance} all="Distance" options={[["1","Within 1 mile"],["5","Within 5 miles"],["10","Within 10 miles"],["25","Within 25 miles"]]} disabled={myLat==null||myLng==null}/></FilterRow>{directoryPending?<Empty>Loading studios…</Empty>:shownStudios.length?<StudioGrid studios={shownStudios}/>:<Empty>There are no studios matching these filters.</Empty>}</>}
    {tab==="groups"&&<><FilterRow><Filter label="Purpose" value={purpose} onChange={setPurpose} all="Purpose" options={[["plan","Plan together"],["community","Community"],["event","Events"]]}/><Filter label="Sort" value={groupSort} onChange={setGroupSort} all="Newest" options={[["name","Name"]]}/></FilterRow>{directoryPending?<Empty>Loading groups…</Empty>:shownGroups.length?<GroupGrid groups={shownGroups}/>:groupRows.length?<Empty>There are no groups matching these filters.</Empty>:<div className="discover-groups-empty"><span><Icon name="groups" size={32}/></span><h2>Plan fitness together</h2><p>Groups are shared calendars and updates for the people you train with. Add classes, invite members, and keep everyone&rsquo;s plans in one place.</p><button type="button" className="btn si" onClick={()=>setGroupCreateOpen(true)}><Icon name="add" size={21}/>Create a group</button></div>}</>}
...
function Filter({label,value,onChange,all,options,disabled=false}:{label:string;value:string;onChange:(value:string)=>void;all:string;options:(string|readonly[string,string])[];disabled?:boolean}){return <label><select aria-label={label} value={value} onChange={(event)=>onChange(event.target.value)} disabled={disabled}><option value="">{all}</option>{options.map((option)=>{const [value,label]=typeof option==="string"?[option,option]:option;return <option value={value} key={value}>{label}</option>})}</select></label>}
