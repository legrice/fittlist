...
export function DiscoverList({ people,studios=[],cities,myLat=null,myLng=null,startHalf,upcoming=[],groups=[],backHref,hideBack=false,groupFrom="discover-groups" }: { people:DirPerson[];studios?:DirStudio[];cities:string[];myCity?:string|null;myLat?:number|null;myLng?:number|null;startHalf?:DiscoverHalf;upcoming?:BrowseDay[];groups?:Group[];backHref:string;hideBack?:boolean;groupFrom?:"discover-groups"|"calendar-following" }) {
  ...
  const shownPeople=peopleRows.filter((person)=>(!discipline||person.disciplines.includes(discipline))&&(!peopleDistance||(geo&&person.lat!=null&&person.lng!=null&&milesBetween(geo.lat,geo.lng,person.lat,person.lng)<=Number(peopleDistance))));
  const studioTypes=[...new Set(studioRows.flatMap((studio)=>studio.types))].sort();
  const shownStudios=studioRows.filter((studio)=>(!placeKind||studio.placeKind===placeKind)&&(!studioType||studio.types.includes(studioType))&&(!studioDistance||(geo&&studio.lat!=null&&studio.lng!=null&&milesBetween(geo.lat,geo.lng,studio.lat,studio.lng)<=Number(studioDistance))));
  const shownGroups=groupRows.filter((group)=>(!purpose||group.purpose===purpose)&&(!groupDistance||(geo&&group.lat!=null&&group.lng!=null&&milesBetween(geo.lat,geo.lng,group.lat,group.lng)<=Number(groupDistance)))).sort((a,b)=>groupSort==="name"?a.name.localeCompare(b.name):0);
