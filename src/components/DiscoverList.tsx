...
export function DiscoverList({ people,studios=[],cities,myLat=null,myLng=null,startHalf,upcoming=[],groups=[],backHref,hideBack=false }: { people:DirPerson[];studios?:DirStudio[];cities:string[];myCity?:string|null;myLat?:number|null;myLng?:number|null;startHalf?:DiscoverHalf;upcoming?:BrowseDay[];groups?:Group[];backHref:string;hideBack?:boolean }) {
  ...
  const discoverCalendarDays:WeekDayRows[]=filteredUpcomingDays.map((day)=>({iso:day.iso,label:day.label,rows:day.items.map((item)=>{const isSaved=!!saved[`${item.classId}|${item.iso}`];return {key:`${item.classId}.${item.iso}`,name:item.name,where:item.where||"Location to come",hm:item.hm,ap:item.ap,coach:{id:item.classId,name:item.attributionName,color:item.coachColor,photo:item.coachPhoto},tag:item.classType||undefined,onTap:()=>void openEvent(item),corner:item.own?undefined:<button type="button" aria-label={isSaved?`Manage saved class ${item.name}`:`Save ${item.name} to your calendar`} className={`calendar-overlay-save${isSaved?" saved":""}`} disabled={savePending} onClick={()=>isSaved?openSavedConfirmation(item):saveClass(item)}><Icon name={isSaved?"check":"add"} size={17}/>{isSaved?"Saved":"Save"}</button>};})}));
  useEffect(()=>{
