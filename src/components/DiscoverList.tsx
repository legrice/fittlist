...
function DiscoverEvent({item,teacher,onOpen}:{item:UpcomingItem;teacher?:DirPerson;onOpen:()=>void}){return <Link className="discover-event-card" href={`/${item.base}/${item.classId}?d=${item.iso}&from=discover`} onClick={(event)=>{if(event.metaKey||event.ctrlKey||event.shiftKey||event.altKey)return;event.preventDefault();onOpen();}}><small>{item.day} • {item.hm}{item.ap.toLowerCase()}</small>{item.classType&&<span className="discover-event-type">{item.classType}</span>}<strong>{item.name}</strong><span className="discover-event-studio">{item.where||"Location to come"}</span><span className="discover-event-teacher"><span className="discover-event-avatar" style={{background:teacher?.color}}>{teacher?.photo?<img src={teacher.photo} alt=""/>:(item.attributionName.trim().charAt(0)||"?").toUpperCase()}</span><span>{item.attributionName}</span></span></Link>}
