...
const SUMMARY_VOICES: { value:SummaryVoice; label:string; description:string; emoji:string }[] = [
  { value:"straightforward", label:"Straightforward", description:"Clear, useful, and to the point.", emoji:"😐" },
  { value:"friendly", label:"Friendly", description:"Warm encouragement without laying it on thick.", emoji:"🙂" },
  { value:"sassy", label:"Roast me", description:"Aggressively sarcastic and unnecessarily personal.", emoji:"😏" },
  { value:"unfiltered", label:"Unhinged", description:"No slurs. No restraint. Completely fucking ridiculous.", emoji:"🤪" },
  { value:"shakespearean", label:"Shakespearean", description:"Forsooth, thy calendar awaits.", emoji:"🧐" },
];
...
export function CalendarScreen({
  ...
  const calendarWeekSummary = useMemo(() => {
    ...
    if (summaryVoice === "friendly")
      ...
      if (totalScheduled === 0)
        title=variant([`${roastContext} What the fuck is this, a calendar or the untouched menu at a restaurant you hate?`,`${roastContext} Holy shit, this schedule is emptier than a motivational quote after you remove the bullshit.`,`${roastContext} Not a goddamn thing. Your calendar has entered the witness protection program.`,`${roastContext} Absolutely fuck-all is happening. Even the tumbleweeds called this place depressing.`,`${roastContext} Jesus tap-dancing Christ, schedule something before this app files a missing-person report.`]);
      else if (totalScheduled < 3)
        title=variant([`${roastContext} One tiny-ass commitment standing there like it pays the fucking rent.`,`${roastContext} What is this adorable little bullshit, a schedule for ants with excellent boundaries?`,`${roastContext} You cracked open the whole damn calendar for this microscopic fitness appetizer.`,`${roastContext} That is not a packed week. That is a polite suggestion wearing gym clothes.`,`${roastContext} Fucking breathtaking. You scheduled almost enough activity to inconvenience a houseplant.`]);
      else if (totalScheduled < 7)
        title=variant([`${roastContext} This schedule is built like a damn brick shithouse: sturdy, loud, and somehow taking up the entire fucking room.`,`${roastContext} Holy fucking shit, look at you stacking classes like the rent is due and burpees are legal tender.`,`${roastContext} This beautiful batshit calendar has structure, drama, and absolutely no interest in calming the fuck down.`,`${roastContext} Jesus tap-dancing Christ, you made a whole fitness tasting menu and apparently you’re every fucking course.`,`${roastContext} Well slap a heart-rate monitor on a tornado, because this week has decided to get shit done.`]);
      else
        title=variant([`${roastContext} Holy fucking shit, this is not a schedule. It is a cry for help laminated by a personal trainer.`,`${roastContext} Your calendar is absolutely batshit, built like a brick shithouse, and apparently powered by caffeine, spite, and whatever the fuck fear smells like.`,`${roastContext} Jesus tap-dancing Christ, leave one goddamn class for somebody whose knees still make normal sounds.`,`${roastContext} This week is so fucking packed it needs a fire marshal, a structural engineer, and an exorcist.`,`${roastContext} You absolute maniac. This schedule just kicked down the door, ate everyone’s protein bars, and demanded another fucking time slot.`]);
    }
  ...
  return (
    <>
      ...
      {!sheet && <><div className={`calendar-scope-top${classSheetDismissed ? " is-expanded" : ""}`} style={{ "--sheet-pull-progress": Math.min(classSheetPullY / 120, 1), "--sheet-search-scale": 1 - (.12 * Math.min(classSheetPullY / 120, 1)) } as React.CSSProperties}>
        ...
        <section className={`calendar-scope-hero calendar-transition-surface${scopeTarget !== "you" ? " calendar-surface-leaving" : ""}${scopeSummaryEntering ? " calendar-surface-entering" : ""}`}><section className="calendar-section-summary personal-upcoming-summary" aria-label="Calendar summary"><button type="button" className={`calendar-summary-copy${summaryVoice === "sassy" ? " is-sassy" : ""}`} aria-label="Change calendar voice" onClick={() => setSummaryVoiceOpen(true)}><strong>{calendarWeekSummary.title}</strong></button><button type="button" className={`calendar-summary-reveal${classSheetDismissed ? " is-open" : ""}`} aria-label={classSheetDismissed ? "Show calendar actions" : "Show your calendar"} aria-expanded={classSheetDismissed} onClick={() => classSheetDismissed ? restoreActionSurface() : setClassSheetDismissed(true)}><Icon name="expand_more" size={25} /></button></section></section></>}
