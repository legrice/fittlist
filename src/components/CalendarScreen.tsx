  ...
  return (
    <>
      ...
      {!sheet && <><div className="calendar-scope-top">
        ...
        <section className="calendar-scope-hero"><section className="calendar-section-summary personal-upcoming-summary" aria-label="Calendar summary"><div className="calendar-summary-copy"><span>{calendarSummary.eyebrow}</span><strong>{calendarSummary.title}</strong>{classSheetDismissed && <small>{calendarSummary.detail}</small>}</div><button type="button" className="calendar-summary-share" onClick={openShare}>Share</button></section>{classSheetDismissed && <section className="calendar-reveal-panel" aria-label="Your calendar tools"><div className="calendar-reveal-actions"><button type="button" onClick={() => setCalendarChooserOpen(true)}>Manage calendars</button><button type="button" onClick={openAdd}>Add to calendar</button></div><div className="calendar-reveal-calendars"><h2>Your calendars</h2><button type="button" onClick={() => setCalendarChooserOpen(true)}><span className="calendar-reveal-icon" style={{ background:viewer.color }}>{viewer.photo ? <img src={viewer.photo} alt="" /> : viewer.name.charAt(0)}</span><span><strong>Personal calendar</strong><small>Your classes, shifts, and saved classes</small></span><Icon name="chevron_right" size={19} /></button>{managedCalendars.map((calendar) => <Link href={calendar.kind === "studio" ? `/s/${calendar.slug}/manage/calendar` : `/g/${calendar.slug}`} key={`${calendar.kind}:${calendar.id}`}><span className={`calendar-reveal-icon ${calendar.kind}`}>{calendar.photo ? <img src={calendar.photo} alt="" /> : <Icon name={calendar.kind === "studio" ? "storefront" : "groups"} size={20} />}</span><span><strong>{calendar.name}</strong><small>{calendar.kind === "studio" ? "Studio calendar" : "Group calendar"}</small></span><Icon name="chevron_right" size={19} /></Link>)}</div></section>}</section></>}
