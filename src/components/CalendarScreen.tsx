...
export function CalendarScreen({
  ...
  return (
    <>
      ...
      {sheet ? <div className="calendar-bottom-actions" aria-label="Schedule actions"><button className="calendar-bottom-add" aria-label="Add to your schedule" onClick={openAdd}><Icon name="add" size={30} /></button></div> : classSheetDismissed && <BodyPortal><div className="calendar-revealed-controls" aria-label="Calendar controls"><div className="calendar-view-toggle" data-active={view} role="group" aria-label="Calendar view"><button type="button" aria-pressed={view === "list"} onClick={() => setView("list")}>Day</button><button type="button" aria-pressed={view === "month"} onClick={() => setView("month")}>Month</button></div><button className="calendar-add-pill" type="button" onClick={openAdd}><Icon name="add" size={24} />Add</button></div></BodyPortal>}
