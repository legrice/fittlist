...
export function CalendarScreen({
  ...
  const managedMonthItems = useMemo(() => {
    const mapped = new Map<string, MonthCellItem[]>();
    for (const day of activeManagedCalendar?.days ?? []) {
      mapped.set(day.iso, day.rows.map((row) => ({
        kind:row.tagTone === "shift" ? "shift" as const : "coaching" as const,
        name:row.name,
        at:atOf(row),
      })));
    }
  ...
  const openDay = useCallback((iso: string) => {
    if (!displayedMonthItems.has(iso)) {
      if (activeManagedCalendar) return;
      ensureComposer();
  ...
  }, [ensureComposer, displayedMonthItems, activeManagedCalendar, todayIso, dayHorizon]);
  ...
  return (
    <>
      ...
      {sheet ? <div className="calendar-bottom-actions" aria-label="Schedule actions"><button className="calendar-bottom-add" aria-label="Add to your schedule" onClick={openAdd}><Icon name="add" size={30} /></button></div> : <BodyPortal><div className="calendar-bottom-actions calendar-tab-add-actions" aria-label="Schedule actions"><button className="calendar-bottom-add" aria-label="Add to your schedule" onClick={openAdd}><Icon name="add" size={30} /></button></div></BodyPortal>}
      {addChoice && (
