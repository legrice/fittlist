...
import { isDesktopViewport, useDesktopLayout } from "@/lib/use-desktop-layout";
...
export function CalendarScreen({
  ...
  const openShare = (_event: ReactMouseEvent<HTMLButtonElement>) => {
    if (isDesktopViewport()) router.push(member ? "/membershare" : "/coachshare");
  ...
  const calendarWeekSummary = useMemo(() => {
    ...
    if (summaryVoice === "friendly")
      ...
      title=coaching && attending ? variant([`Hark! Thou art teaching ${coaching} ${classWord(coaching)} and attending ${attending} this week.`,`Lo, this week bears ${coaching} ${classWord(coaching)} to teach and ${attending} to attend.`,`By my troth, thou teachest ${coaching} and attendest ${attending} ${classWord(attending)} this week.`]) : coaching ? variant([`Hark! Thou art teaching ${coaching} ${classWord(coaching)} this week.`,`Lo, this week bears ${coaching} ${classWord(coaching)} for thee to teach.`,`By my troth, thou teachest ${coaching} ${classWord(coaching)} this week.`]) : attending ? `Hark! Thou art attending ${attending} ${classWord(attending)} this week.` : personal ? `Thou hast ${personal} ${personal === 1 ? "workout" : "workouts"} planned this week.` : "Thou hast nothing scheduled this week.";
    if (summaryVoice === "shakespearean" && personal && (coaching || attending))
      title+=` Thou hast also ${personal} ${personal === 1 ? "workout" : "workouts"} planned.`;
  ...
  const days: WeekDayRows[] = useMemo(() => {
    ...
    return out;
  }, [desktop, classes, todayIso, studioById, handle, visible.coaching, visible.personal, visible.saved, savedByIso, viewer, dayHorizon]);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
      if (entries.some((entry) => entry.isIntersecting) && days.length > lastAutoDayCount.current) {
        lastAutoDayCount.current = days.length;
        setDayHorizon((value) => value + 84);
      }
...
<label className="calendar-desktop-filter">
  ...
  return (
    <>
      ...
      {!desktop && !sheet && !classSheetDismissed && <section className={`calendar-action-sheet calendar-pull-sheet calendar-transition-surface${restoring ? " calendar-front-restoring" : ""}${scopeTarget !== "you" ? " calendar-surface-leaving" : ""}${scopeSummaryEntering ? " calendar-surface-entering" : ""}`} ref={frontSheetRef} aria-label="Calendar actions">
        <div className="calendar-action-hub">
          ...
          {managedStudios.length > 0 && <section><div className="calendar-action-section-head"><h3>Studios</h3></div><div className="calendar-action-list">
            {managedStudios.map((studio) => <Link key={studio.id} href={studio.admin ? `/s/${studio.slug}/manage` : `/s/${studio.slug}`}><span className="calendar-action-icon studio">{studio.photo ? <img src={studio.photo} alt="" /> : <Icon name="storefront" size={23} />}</span><span><small className="calendar-relationship-role">{studio.admin ? "Admin" : "Coach"}</small><strong>{studio.name}</strong></span><Icon name="chevron_right" size={20} /></Link>)}
          </div></section>}
          <section><div className="calendar-action-section-head"><h3>Groups</h3>{groupCalendars.length > 0 && <button type="button" onClick={() => setCreateGroupOpen(true)}><Icon name="add" size={17} />New group</button>}</div><div className="calendar-action-list">
            {groupCalendars.map((group) => <Link key={group.id} href={`/g/${group.slug}`}><span className="calendar-action-icon group">{group.photo ? <img src={group.photo} alt="" /> : <Icon name="groups" size={23} />}</span><span><small className="calendar-relationship-role">{group.role === "owner" || group.role === "admin" ? "Admin" : "Member"}</small><strong>{group.name}</strong></span><Icon name="chevron_right" size={20} /></Link>)}
      ...
      {(desktop || sheet || classSheetDismissed) && !bare && days.length > 0 && view === "month" && (
        <ScrollHead
          on={scrolled}
      ...
      {sheet ? <div className="calendar-bottom-actions" aria-label="Schedule actions"><button className="calendar-bottom-add" aria-label="Add to your schedule" onClick={openAdd}><Icon name="add" size={30} /></button></div> : !desktop && classSheetDismissed && <BodyPortal><div className={`calendar-revealed-controls${returning ? " is-returning" : ""}`} aria-label="Calendar controls"><button className="calendar-add-pill" type="button" aria-label="Add to your calendar" onClick={openAdd}><Icon name="add" size={36} /></button></div></BodyPortal>}
