...
export function CalendarScreen({
  ...
  const calendarWeekSummary = useMemo(() => {
    ...
    if (summaryVoice === "friendly")
      ...
      title=coaching ? variant([`Oh, incredible, you’re coaching ${coaching} ${classWord(coaching)}${studioPhrase}${attendingPhrase}, because apparently having a normal week was simply too boring for the relentlessly booked and suspiciously energetic main character you’ve decided to become.`,`You’ve somehow volunteered to coach ${coaching} ${classWord(coaching)}${studioPhrase}${attendingPhrase}, which is a bold schedule for someone who will absolutely insist they are “taking it easy” while sprinting between studios with six percent battery.`,`Look at you, coaching ${coaching} ${classWord(coaching)}${studioPhrase}${attendingPhrase}, bravely proving once again that boundaries are merely a cute suggestion and an empty calendar is your greatest personal enemy.`,`This week you’re coaching ${coaching} ${classWord(coaching)}${studioPhrase}${attendingPhrase}, a completely reasonable plan if your hobbies include overcommitting, carrying three beverages, and saying “I’m fine” with the haunted confidence of a fitness professional.`,`Congratulations, you’ve packed in ${coaching} ${classWord(coaching)} to coach${studioPhrase}${attendingPhrase}, because rest is apparently for civilians and your calendar needed one more opportunity to publicly question your decision-making.`]) : attending ? variant([`You’re attending ${attending} ${classWord(attending)} this week, which is adorable considering how confidently you’ll call this “recovery.”`,`Apparently you’re attending ${attending} ${classWord(attending)} this week, because quietly relaxing without turning it into an organized fitness commitment would be far too easy.`,`You booked ${attending} ${classWord(attending)} this week, a heroic act of optimism from someone who will begin negotiating with the alarm the moment it goes off.`,`You’re attending ${attending} ${classWord(attending)} this week, so please enjoy the brief fantasy that you planned adequate travel time between any of them.`,`There are ${attending} ${classWord(attending)} on your calendar this week, because your favorite form of self-care is apparently another appointment.`]) : personal ? `${personal} personal ${classWord(personal)} this week, a stunning display of organization from someone who definitely will not remember what any of them are for.` : "Nothing booked this week, which is either personal growth or the calendar finally staging an intervention.";
