...
export function GroupJoinButton({ groupId, initial, signedIn }: { groupId: string; initial: boolean; signedIn: boolean }) {
  ...
  return <button className={`btn ${joined ? "si" : "ghost"}`} disabled={pending} onClick={() => {
    ...
    start(async () => {
      const result = await setGroupMembership(groupId, !joined);
