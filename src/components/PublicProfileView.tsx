...
export async function PublicProfileView({
  ...
  // Public profiles are separate from You, including your own profile.
  const backTo = isOwner
    ? { href: "/you", label: "Back to You" }
    ...
    <>
      ...
      {user.highlights.length > 0 && (
        <div className="profsec focussec">
          <h2 className="prof-sec-h">Teaching focus</h2>
    ...
    >
      <div className="profwrap">
        ...
        <ProfileTabs
          ...
          ownerTop={<ProfileOverflow profileId={user.id} path={`/${handle}`} name={user.name} canReport={!isOwner && !!viewerId}>{showContact && <ContactSheet handle={handle} coachName={user.name} signedIn={signedIn} canMessage={canMessage} ways={ways}/>}</ProfileOverflow>}
