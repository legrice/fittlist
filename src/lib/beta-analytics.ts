export const USAGE_KINDS = ["app_active", "calendar_viewed", "explore_viewed", "class_viewed", "screen_error"] as const;
...
export function betaAnalytics(people: AnalyticsPerson[], events: AnalyticsEvent[], excludedEmails: string[], now = new Date(), days = 7) {
  ...
  return {
    ...
    sources: [...sources].map(([source, count]) => ({ source, count })).sort((a, b) => b.count - a.count), daily,
