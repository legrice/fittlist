...
// Daily rollup per trainer (page_visits). Days and the Mon–Sun week follow
// the app's timezone, matching the product's week.
...
export async function recordVisit(trainerUserId: string): Promise<void> {
  ...
  await db
    ...
    .values({ trainerUserId, date: todayIso(), count: 1 })
    .onConflictDoUpdate({
...
export async function recordScheduleOpen(trainerUserId: string): Promise<void> {
  ...
  await db
    .insert(schema.pageVisits)
    .values({ trainerUserId, date: todayIso(), count: 0, scheduleOpens: 1 })
    .onConflictDoUpdate({
      target: [schema.pageVisits.trainerUserId, schema.pageVisits.date],
