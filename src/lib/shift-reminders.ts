...
const zonedTarget = (date: Date, timeZone: string) => {
...
export async function sendShiftReminders(now = new Date()): Promise<{ sent: number }> {
  const targetInstant = new Date(now.getTime() + 12 * 60 * 60 * 1000);
  const targets = new Map<string, ReturnType<typeof zonedTarget>>();
  const targetFor = (timeZone: string) => {
    let target = targets.get(timeZone);
    if (!target) {
      target = zonedTarget(targetInstant, timeZone);
      targets.set(timeZone, target);
    }
    return target;
  };
  const db = await getDb();
  const rows = (await db.select().from(schema.classes)).filter((row) => {
    const target = targetFor(row.timeZone);
    return row.isPublic &&
  ...
  const [covers, studios, closures] = await Promise.all([
    ...
    studioIds.length ? db.select().from(schema.studioClosedDays).where(
      inArray(schema.studioClosedDays.studioId, studioIds),
    ) : [],
  ]);
  const coverByClassDate = new Map(covers.map((cover) => [`${cover.classId}|${cover.occurrenceDate}`, cover.coachUserId]));
  const studioById = new Map(studios.map((studio) => [studio.id, studio]));
  const closed = new Set(closures.map((closure) => `${closure.studioId}|${closure.occurrenceDate}`));
  const assignments = rows.flatMap((row) => {
    const target = targetFor(row.timeZone);
    const coverKey = `${row.id}|${target.iso}`;
    const coachUserId = coverByClassDate.has(coverKey) ? coverByClassDate.get(coverKey) : row.coachUserId;
    return coachUserId && (!row.studioId || !closed.has(`${row.studioId}|${target.iso}`))
      ? [{ row, coachUserId, target }]
      : [];
  });
  ...
  let sent = 0;
  for (const { row, coachUserId, target } of assignments) {
