...
async function main() {
  ...
  assert((await as(1,()=>saveEventSettings(studio.slug!,date,[{id:cls.id,capacity:2,waitlist:true}]))).ok);
  assert((await as(5,()=>registerForEvent(studio.slug!,cls.id,date,true))).ok);
  assert((await as(6,()=>registerForEvent(studio.slug!,cls.id,date,true))).ok);
  await as(5,()=>registerForEvent(studio.slug!,cls.id,date,true));
  const queue=await as(1,()=>eventWaitingRoster(studio.slug!));assert.equal(queue?.length,2,'Waitlist joins are idempotent');
  assert.equal(await as(5,()=>eventWaitingRoster(studio.slug!)),null,'Waitlist emails are private');
  const status=await as(5,()=>eventSchedule(studio.slug!));assert(status?.classes.find(c=>c.id===cls.id)?.waitlisted);assert(!status?.classes.find(c=>c.id===cls.id)?.registered);
  assert(!(await as(1,()=>promoteEventWaitlist(studio.slug!,queue![0].id))).ok,'Cannot promote a full class');
  await as(loser,()=>registerForEvent(studio.slug!,cls.id,date,false));
  assert(!(await as(7,()=>setGoing(cls.id,date,true))).ok,'Ordinary saves cannot jump a waiting queue');
  assert(!(await as(5,()=>promoteEventWaitlist(studio.slug!,queue![0].id))).ok,'Attendees cannot promote themselves');
  assert(!(await as(1,()=>promoteEventWaitlist(studio.slug!,queue![1].id))).ok,'Cannot skip the first waiting person');
  assert((await as(1,()=>promoteEventWaitlist(studio.slug!,queue![0].id))).ok);
  assert.equal((await as(1,()=>eventWaitingRoster(studio.slug!)))?.length,1);
  assert.equal((await as(1,()=>eventSchedule(studio.slug!,true)))?.classes.find(c=>c.id===cls.id)?.count,2);
  const remaining=queue![1];const remainingIndex=people.findIndex(p=>p.id===remaining.userId);
  assert((await as(remainingIndex,()=>registerForEvent(studio.slug!,cls.id,date,false))).ok,'Leaving waitlist works');
  assert.equal((await as(1,()=>eventWaitingRoster(studio.slug!)))?.length,0);
  await as(7,()=>registerForEvent(studio.slug!,cls.id,date,true));
  const declined=(await as(1,()=>eventWaitingRoster(studio.slug!)))![0];
  assert(!(await as(7,()=>removeEventWaitlist(studio.slug!,declined.id))).ok);
  assert((await as(1,()=>removeEventWaitlist(studio.slug!,declined.id))).ok,'Admin can remove someone who declines');
  assert.equal((await as(1,()=>eventWaitingRoster(studio.slug!)))?.length,0);
  await as(1,()=>saveEventSettings(studio.slug!,date,[{id:cls.id,capacity:2,waitlist:false}]));
