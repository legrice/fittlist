...
| 7. Scale | 400 follows, 4,000 class definitions, 20 studios, 20 groups and 1,020 memberships; incremental scroll and month-date reachability. | Passed after fixes; future-month data gap remains |
...
1. **Following's month view reaches beyond its data window.** `loadCalendarRemainder` requests 180 days, but `buildDiscoverFeed` caps its end offset at 30 days. The month UI initially offers twelve months. An empty later date therefore does not prove no classes exist. Add bounded date-window loading with explicit loading/error states before treating those months as complete; do not simply fetch all future class occurrences in the initial response. Empty Following dates also inherit an “Add to” label although their handler only switches to day view.
