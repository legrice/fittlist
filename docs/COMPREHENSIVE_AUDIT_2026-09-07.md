...
8. **Resolved in the September 7 profile-card follow-up: schedule-card alignment.** Person, studio, and group schedules now share a time gutter: bold time and duration on the left, single-line truncated title and location on the right. Supplied coach attribution and avatars appear above the details, including classes attended on member profiles. The gutter expands with enlarged text, and overflow controls retain their own space. Production build, typecheck, lint, and core regressions pass. All 24 existing Chrome visual/reflow checks pass, including 200% text. A focused local audit also passed 27 layout samples (three profile types at 320/390/1440 px in Chrome, WebKit, and Firefox) and six studio/group class-sheet dismissal checks. Studio/group screenshots were inspected. All checks used disposable synthetic fixtures. Inline Save controls on public studio/group schedule cards remain a separate UI follow-up.
